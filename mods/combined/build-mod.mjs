...
export async function transform(
  ...
  if (selected.includes('model-spread')) {
    const spreadInputs = Object.fromEntries(
    ...
    const next = {
      ...composed,
      ...Object.fromEntries(
        Object.entries(spread).map(([name, content]) => [`webview/assets/${name}`, content]),
      ),
    };
    onStage(
      recordChanges('model-spread', 'model-spread adapters', mapOutput(composed), mapOutput(next)),
    );
    composed = next;
  }
  if (selected.includes('theme-icon')) {
    ...
    onStage(
      recordChanges('theme-icon', 'theme-icon adapters', mapOutput(composed), mapOutput(next)),
    );
    composed = next;
  }
  ...
  if (selected.includes('custom-cli')) {
    ...
    onStage(
      recordChanges(
        'custom-cli',
        'custom CLI launch configuration',
        mapOutput(composed),
        mapOutput(next),
      ),
    );
...
export function validateInputs(bundles, mods, { currentSource = false } = {}) {
  for (const [name, hash] of Object.entries(compatibilityFor(mods).files)) {
    if (
      typeof bundles[name] !== 'string' ||
      (!currentSource && crypto.createHash('sha256').update(bundles[name]).digest('hex') !== hash)
    )
      throw Error(`Unsupported bundle: ${name}`);
  }
}
if (import.meta.main) {
  ...
  console.log(
    `Model Spread + Theme Icon: wrote ${Object.keys(patched).length} composed overlay files`,
  );
}
