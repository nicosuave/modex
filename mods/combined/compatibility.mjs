...
export const componentManifests=Object.fromEntries(['model-spread','theme-icon'].map(id=>[id,JSON.parse(fs.readFileSync(new URL(`../${id}/compatibility.json`,import.meta.url),'utf8'))]));
export const defaultMods=Object.keys(componentManifests);
export const legacyMarkers={'model-spread':'webview/assets/model-spread.mjs','theme-icon':'webview/assets/theme-icon-runtime.mjs'};
export function selectMods(mods=defaultMods) {
  if(!Array.isArray(mods)||!mods.length||mods.some(id=>!Object.hasOwn(componentManifests,id))||new Set(mods).size!==mods.length)throw Error(`Select unique known mods: ${defaultMods.join(', ')}`);
  return defaultMods.filter(id=>mods.includes(id));
}
...
export const compatibility=combineCompatibility(Object.values(componentManifests));
