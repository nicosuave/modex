...
export async function main(args = process.argv.slice(2), { mods } = {}) {
  ...
  for (let i = 0; i < args.length; i += 2) {
    if (
      ...
      throw Error(
        'Usage: bun run modex verify [--mods LIST] [--source APP] [--dev-root DIRECTORY]',
      );
  ...
  if (JSON.stringify(first) !== JSON.stringify(second))
    throw Error('Nondeterministic mod transformation');
  const virtual = {};
  if (options['--dev-root']) {
    ...
    for (const id of Object.keys(rendererModules))
      if (manifest.modules[id]) {
        virtual[`webview/assets/modex-development-${id}.mjs`] = rendererSource(
          id,
          readModule(options['--dev-root'], manifest, id),
        );
      }
  }
  ...
  for (const [name, content] of Object.entries({ ...first, ...virtual })) {
    ...
    for (const item of parser.scan(content).imports) {
      ...
      if (
        !Object.hasOwn(first, target) &&
        !Object.hasOwn(virtual, target) &&
        !entryFor(archive, target)
      )
  ...
  try {
    ...
    delete env.COMBINED_BUNDLES;
    delete env.TASK_PANES_BUNDLES;
    delete env.CUSTOM_CLI_BUNDLES;
    delete env.CUSTOM_CLI_EXECUTABLE;
    ...
    if (selected.includes('model-spread') && selected.includes('theme-icon'))
      env.COMBINED_BUNDLES = temporary;
    if (selected.includes('task-panes')) env.TASK_PANES_BUNDLES = temporary;
    if (selected.includes('custom-cli')) {
      env.CUSTOM_CLI_BUNDLES = temporary;
      env.CUSTOM_CLI_EXECUTABLE = path.resolve(source, 'Contents/Resources/codex');
    }
    const suites = [
      'lib',
      'modex.test.mjs',
    ...
    if (result.status !== 0) throw Error('Mod verification tests failed');
  } finally {
    ...
    JSON.stringify(
      {
        ...
        tests: 'passed with selected stock bundle suites',
        launched: false,
      },
      null,
      2,
