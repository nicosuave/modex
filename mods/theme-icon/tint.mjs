...
function rgb(h, s, l) {
  ...
  const m = l - c / 2;
  const parts = [[c,x,0],[x,c,0],[0,c,x],[0,x,c],[x,0,c],[c,0,x]][Math.floor(h * 6) % 6];
...
export function tintIcon(pixels, accent) {
  if (!(pixels instanceof Uint8Array) || pixels.length % 4) throw new TypeError('Expected RGBA bytes');
...
export function tintThemeIcon(pixels, {accent, surface}, variant = 'dark') {
  if (!['dark', 'light'].includes(variant)) throw new TypeError('Expected dark or light source variant');
  ...
  for (let i = 0; i < pixels.length; i += 4) {
    ...
    for (let c=0;c<3;c++) {
      const mapped = level <= anchor ? background[c]*level/anchor
        : background[c]+(1-background[c])*(level-anchor)/(1-anchor);
      output[i+c] = Math.round(output[i+c]*(1-neutralWeight)+255*mapped*neutralWeight);
    }
  }
  return output;
}
