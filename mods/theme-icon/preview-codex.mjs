...
const [resources = '/Applications/ChatGPT.app/Contents/Resources', destination = 'work/codex-theme-preview'] = process.argv.slice(2);
...
const themes = ['gruvbox-dark-medium', 'dracula', 'nord'].map(name => {
  ...
  const matches = assets.filter(file => new RegExp(`^${name}-[a-f0-9]+\\.js$`).test(file) && file === registeredFile);
  ...
  const seed = {
    surface: colors['editor.background'], ink: colors['editor.foreground'],
    accent: colors['activityBarBadge.background'], ...theme.chromeTheme,
  };
