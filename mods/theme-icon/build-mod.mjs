...
export function replaceOnce(source,anchor,value) {
  if(source.split(anchor).length!==2)throw new Error(`Expected exactly one Theme Icon anchor: ${anchor.slice(0,100)}`);
...
export async function transform(bundles) {
  ...
  const main=find('.vite/build/main-'),initial=find('webview/assets/app-initial-'),settings=find('webview/assets/general-settings-');
  const patch=(name,anchor,value)=>output[name]=replaceOnce(output[name],anchor,value);
  output[initial]='import * as ThemeIconRuntime from "./theme-icon-runtime.mjs";'+output[initial];
  patch(initial,'let b=s===`light`?_:y,x=b.fonts.codeFace',
  ...
  output[settings]='import {Settings as ThemeIconSettings} from "./theme-icon-runtime.mjs";'+output[settings];
  patch(settings,',C}function Lo(e){',
  ...
  patch(main,'I=e=>{if(e===`app-default`&&t!==a.a.Dev)',
    'I=e=>{if(ThemeIconMain.apply(e))return;if(e===`app-default`&&t!==a.a.Dev)');
  patch(main,'};if(g){ee();let e=()=>{let e=A();e===`codex-system`&&I(e)};',
    '};ThemeIconMain.configure(l,process.resourcesPath,()=>I(A()));if(g){ee();let e=()=>{let e=A();e===`codex-system`&&I(e)};');
  patch(main,'case`persisted-atom-sync-request`:this.sendPersistedAtomState(e,t.responsePriority);',
  ...
  for(const file of ['runtime','state','render','tint','palette']) {
    ...
    for(const dependency of ['runtime','state','render','tint','palette'])content=content.replaceAll(`'./${dependency}.mjs'`,`'./theme-icon-${dependency}.mjs'`);
    output[`webview/assets/theme-icon-${file}.mjs`]=content;
  }
  const build=await Bun.build({entrypoints:[path.join(here,'main.mjs')],target:'node',format:'cjs',minify:false,write:false});
...
if(import.meta.main) {
  ...
  for(const [name,hash]of Object.entries(manifest.files)) {
    ...
    if(crypto.createHash('sha256').update(bytes).digest('hex')!==hash)throw new Error(`Unsupported bundle: ${name}`);
    bundles[name]=bytes.toString();
  }
  const patched=await transform(bundles);
  for(const [name,content]of Object.entries(patched)) {
    const destination=path.join(output,name);fs.mkdirSync(path.dirname(destination),{recursive:true});fs.writeFileSync(destination,content);
  }
