...
export function normalizeSettings(value) {
  ...
  if (value?.version === 1 && value.variants && typeof value.variants === 'object') {
    for (const [key, variant] of Object.entries(value.variants).slice(0, 512)) {
      if (/^(light|dark):.{1,128}$/.test(key) && VARIANTS.some((v) => v.id === variant))
        variants[key] = variant;
    }
  }
  const colors = {};
  if (value?.version === 1)
    for (const [key, color] of Object.entries(value.colors ?? {}).slice(0, 512)) {
      if (
        /^(light|dark):.{1,128}$/.test(key) &&
        typeof color === 'string' &&
        /^[a-zA-Z0-9._-]{1,128}$/.test(color)
      )
        colors[key] = color;
    }
  return { version: 1, variants, ...(Object.keys(colors).length ? { colors } : {}) };
}
export function selectedColor(value, key) {
  return normalizeSettings(value).colors?.[key] ?? 'accent';
}
export function setColor(value, key, color) {
  if (
    ...
    throw new TypeError('Invalid palette choice');
  const next = normalizeSettings(value);
  next.colors = { ...next.colors, [key]: color };
  return next;
}
export function selectedVariant(value, key) {
  return normalizeSettings(value).variants[key] ?? 'theme';
}
...
export function validatePayload(value) {
  if (
    !value ||
    !['dark', 'light'].includes(value.appearance) ||
    !VARIANTS.some((v) => v.id === value.variant)
  )
    return null;
