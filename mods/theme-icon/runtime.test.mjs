...
test('root theme subscription sends actual colors, reacts to custom changes, persisted choice and focus', async () => {
  ...
  function Host({ theme, id = 'gruvbox', appearance = 'dark' }) {
    useTheme(React, { theme, id, appearance, read, write, listen, bridge });
    return null;
  }
  ...
  try {
    await act(async () => {
      root = TestRenderer.create(
        React.createElement(Host, { theme: { surface: '#282828', accent: '#458588' } }),
      );
    });
    await act(settle);
    assert.deepEqual(messages.at(-1), {
      type: 'modex-theme-icon',
      appearance: 'dark',
      variant: 'theme',
      surface: '#282828',
      accent: '#458588',
    ...
    await act(async () =>
      root.update(React.createElement(Host, { theme: { surface: '#323020', accent: '#aacc11' } })),
    );
    await act(settle);
    assert.equal(messages.at(-1).accent, '#aacc11');
    assert.equal(messages.at(-1).surface, '#323020');
    await act(async () => write(KEY, setVariant(settings, 'dark:gruvbox', 'light')));
    await act(settle);
    assert.equal(messages.at(-1).variant, 'light');
    ...
    await act(async () =>
      root.update(
        React.createElement(Host, {
          id: 'nord',
          appearance: 'light',
          theme: { surface: '#ffffff', accent: '#88c0d0' },
        }),
      ),
    );
    await act(settle);
    assert.equal(messages.at(-1).variant, 'theme');
    assert.equal(messages.at(-1).appearance, 'light');
...
test('settings offers each variant, persists selection and reports stock preference save failures', async () => {
  ...
  const Row = ({ label, description, control }) =>
    React.createElement('section', null, label, description, control);
  const Dropdown = ({ triggerButton, children }) =>
    React.createElement('div', null, triggerButton, children);
  const DropdownButton = (props) => React.createElement('button', props);
  const CheckIcon = () => null;
  const Menu = {
    Section: ({ children }) => React.createElement('div', null, children),
  ...
  function Host({ enabled = false }) {
    ...
    return React.createElement(Settings, {
      ...
      CheckIcon,
      enabled,
      previews: { codexDark: 'test-dark', codexLight: 'test-light' },
      onEnable: async () => {
        enableCalls++;
        throw new Error('Could not save Dock preference');
      },
    });
  }
  try {
    ...
    assert.deepEqual(
      radios.map((r) => r.props['aria-label']),
      ['Accent', 'Foreground', 'Highlight', 'Added', 'Removed'],
    );
    ...
    assert.deepEqual(
      ...
      ['Theme background', 'Dark tile', 'Light tile', 'Original'],
    );
    ...
    assert.equal(settings.variants['dark:gruvbox'], 'light');
    assert.equal(enableCalls, 1);
    assert.equal(
      root.root.findByProps({ role: 'alert' }).children[0],
      'Could not save Dock preference',
    );
    await act(async () => root.update(React.createElement(Host, { enabled: true })));
    await act(async () => {
      root.root.findAllByProps({ name: 'modex-theme-icon-color' })[3].props.onChange();
    });
    assert.equal(settings.colors['dark:gruvbox'], 'semantic.diffAdded');
    assert.equal(
      root.root.findAllByProps({ name: 'modex-theme-icon-color' })[3].props.checked,
      true,
    );
    assert.equal(enableCalls, 1);
    await act(async () => root.unmount());
