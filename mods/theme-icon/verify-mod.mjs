...
try {
  if (args.length && !(args.length === 2 && args[0] === '--source'))
    throw new Error('Usage: bun verify-mod.mjs [--source APP]');
  ...
  const first = await transform(originals),
    second = await transform(originals);
  if (JSON.stringify(first) !== JSON.stringify(second))
    throw new Error('Nondeterministic transformation');
  ...
  for (const [name, source] of Object.entries(first)) {
    ...
    for (const item of parser.scan(source).imports) {
      ...
      if (!Object.hasOwn(first, target) && !entryFor(archive, target))
        throw new Error(`Missing import ${item.path} in ${name}`);
    }
  }
  ...
  try {
    ...
    const result = spawnSync(
      ...
      [
        ...
        'mods/theme-icon',
        'mods/app-tools-auth',
        'lib/prepare-mod.test.mjs',
        'mods/model-spread/prepare-mod.test.mjs',
      ],
      {
        cwd: path.resolve(import.meta.dirname, '../..'),
        env: { ...process.env, THEME_ICON_BUNDLES: temporary, APP_TOOLS_AUTH_SOURCE: source },
        stdio: 'inherit',
      },
    );
    if (result.error) throw result.error;
    if (result.status !== 0) throw new Error('Theme Icon tests failed');
  } finally {
