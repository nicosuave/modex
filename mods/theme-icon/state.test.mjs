...
import {KEY,themeKey,selectedVariant,setVariant,normalizeSettings,validatePayload} from './state.mjs';
...
test('variant persists by Codex theme and appearance, independent of custom colors',()=>{
  const gruvbox=themeKey('gruvbox','dark'), light=themeKey('gruvbox','light'), nord=themeKey('nord','dark');
...
test('invalid and future preferences fall back without executing arbitrary variants',()=>{
  assert.equal(selectedVariant({version:2,variants:{'dark:nord':'light'}},'dark:nord'),'theme');
  assert.deepEqual(normalizeSettings({version:1,variants:{'dark:nord':'bad','other:nord':'light'}}),{version:1,variants:{}});
  ...
  for(const invalid of [{...valid,accent:'url(x)'},{...valid,variant:'path'},{...valid,surface:'#fff'},{...valid,appearance:'system'},null])assert.equal(validatePayload(invalid),null);
});
test('all four variants select the right source and preserve original pixels',()=>{
  ...
  assert.deepEqual(original,bytes);assert.notEqual(original,bytes);
...
test('compatibility supports native and renderer bundles without traversal',()=>{
  ...
  for(const name of ['/tmp/x.js','../x.js','a/../b.js','a//b.js','a/./b.js','a\\b.js','a\0b.js'])assert.throws(()=>compatibilityEntryPath(name));
});
