...
const [
  ...
  destination = 'work/theme-icon-preview',
  accent,
] = process.argv.slice(2);
const colors = accent
  ? [accent]
  : ['#6680ff', '#d99b42', '#4ca982', '#dc759c', '#9c7be8', '#888888'];
fs.mkdirSync(destination, { recursive: true });
const size = 256,
  gap = 16,
  width = colors.length * (size + gap) + gap;
...
for (const [row, mode] of ['dark', 'light'].entries()) {
  ...
  for (const [col, color] of colors.entries()) {
    ...
    fs.writeFileSync(
      path.join(destination, `${mode}-${color.slice(1)}.png`),
      PNG.sync.write(output),
    );
    const originX = gap + col * (size + gap),
      originY = gap + row * (size + gap + 12);
    ...
    for (let y = 0; y < 6; y++)
      for (let x = 32; x < size - 32; x++) {
        const to = ((originY + size + 4 + y) * width + originX + x) * 4;
        sheet.data.set(
          [
            parseInt(swatch.slice(0, 2), 16),
            parseInt(swatch.slice(2, 4), 16),
            parseInt(swatch.slice(4, 6), 16),
            255,
          ],
          to,
        );
