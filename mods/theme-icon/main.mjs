...
export function setRenderer(next) {
  if(typeof next?.renderPixels!=='function'||typeof next?.sourceVariant!=='function')throw Error('Invalid Theme Icon renderer');
  ...
  for(const appearance of ['dark','light'])for(const variant of ['theme','dark','light','original']) {
    if(!['dark','light'].includes(next.sourceVariant(variant,appearance)))throw Error('Invalid Theme Icon source variant');
    checkedPixels(next.renderPixels(sample,{accent:'#336699',surface:'#222222'},variant,appearance),sample.length);
  }
...
function checkedPixels(pixels,length) {
  if(!(pixels instanceof Uint8Array)&&!(pixels instanceof Uint8ClampedArray)||pixels.length!==length)throw Error('Invalid Theme Icon pixel output');
  return pixels;
}
function createImage(value,implementation) {
  ...
  output.data=Buffer.from(checkedPixels(implementation.renderPixels(source.data,value,value.variant,value.appearance),source.data.length));
