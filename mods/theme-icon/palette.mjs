...
export function paletteChoices(theme) {
  ...
  for(const [id,label,read]of ROLES) {
    ...
    if(choices.some(c=>Math.hypot(...parseAccent(c.color).map((v,i)=>(v-rgb[i])*255))<24))continue;
    choices.push({id,label,color:color.toLowerCase()});
  }
  return choices;
}
export function chosenColor(choices,id){return choices.find(c=>c.id===id)??choices[0];}
