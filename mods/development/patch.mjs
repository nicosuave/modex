...
const compatibility=JSON.parse(fs.readFileSync(new URL('./compatibility.json',import.meta.url),'utf8'));
export const protocolPath=Object.keys(compatibility.files)[0];
export function readDevelopmentProtocol(source) {return inspectCompatibility(source,compatibility).bundles.get(protocolPath);}
...
export function transformProtocol(code) {
  if(code.split(protocolAnchor).length!==2)throw Error('Unsupported development protocol call site');
  return code.replace(protocolAnchor,'function $e(e){rt(),o.protocol.handle(`app`,async t=>{const modexResponse=require(`./modex-development-main.cjs`).responseFor(t);if(modexResponse)return modexResponse;let n=Ze(t.url,e);');
}
export function inspectDevelopmentForBuild(root,mods,source) {
  ...
  return inspectDevelopment(root,{hookHash:computeHookHash(mods),mods,source:sourceForMods(mods)});
}
export function rendererShim(id,{file,exports:exportsList}) {
  const bundled=file.replace(/\.mjs$/,'-bundled.mjs');
  const persistence=id==='model-spread'?`import {storage,target} from './model-spread-storage.mjs';\n`:'';
...
export async function applyDevelopment(replacements,root,mods,protocolOriginal) {
  ...
  if(!protocolOriginal||digest(protocolOriginal)!==compatibility.files[protocolPath])throw Error('Unsupported development protocol source hash');
  if(replacements.has(protocolPath))throw Error('Unexpected development protocol transform owner');
  ...
  for(const id of Object.keys(rendererModules)) {
    ...
    const module=rendererModules[id],filename=`webview/assets/${module.file}`;
  ...
  const callback=mods.includes('theme-icon')?`,onIconRenderer:renderer=>require('./theme-icon-main.cjs').setRenderer(renderer)`:'';
  ...
  return {root:path.resolve(root),mods,hookHash:manifest.hookHash,moduleHashes:Object.fromEntries(Object.entries(manifest.modules).map(([id,module])=>[id,module.hash]))};
}
