...
function temporary() {
  return fs.mkdtempSync(path.join(os.tmpdir(), 'modex-dev-test-'));
}
function publish(root, manifest) {
...
test('publishes repeatable complete generations and bundles runtime modules', async () => {
  ...
  try {
    ...
    const modelSource = fs.readFileSync(
      path.join(root, first.modules['model-spread'].file),
      'utf8',
    );
    const model = { exports: {} };
    new Function('module', 'exports', modelSource)(model, model.exports);
    expect(Object.keys(model.exports).length).toBeGreaterThan(0);
    const runtime = await buildRuntime();
    expect(Object.keys(runtime)).toEqual(['main.cjs']);
...
test('rejects manifest, tampered bytes, escaped files and mismatched hooks', async () => {
  ...
  try {
    ...
    expect(() => inspectDevelopment(root, { ...manifest, hookHash: '0'.repeat(64) })).toThrow(
      'repackage',
    );
    ...
    const entry = manifest.modules['theme-icon-render'],
      file = path.join(root, entry.file),
      original = fs.readFileSync(file);
    fs.writeFileSync(file, 'corrupted');
    expect(() => inspectDevelopment(root)).toThrow('hash mismatch');
    fs.rmSync(file);
    fs.writeFileSync(path.join(outside, 'module.cjs'), original);
    fs.symlinkSync(path.join(outside, 'module.cjs'), file);
    expect(() => inspectDevelopment(root)).toThrow('escapes root');
    publish(root, {
      ...manifest,
      modules: { ...manifest.modules, 'theme-icon-render': { ...entry, file: '../outside.cjs' } },
    });
...
test('refresh adopts valid pure renderer, retains last good on failure, never reloads windows', async () => {
  ...
  try {
    ...
    const refresh = createRefresh({
      root,
      expected: manifest,
      onIconRenderer: (value) => {
        renderer = value;
        calls++;
      },
      log: { error: (...values) => errors.push(values), info: (value) => messages.push(value) },
    });
    expect(refresh()).toEqual(manifest);
    expect(calls).toBe(1);
    const prior = renderer,
      entry = manifest.modules['theme-icon-render'];
    ...
    expect(refresh()).toEqual(manifest);
    expect(renderer).toBe(prior);
    expect(calls).toBe(1);
    refresh();
    expect(errors).toHaveLength(1);
    expect(refresh.readModule('theme-icon-runtime')).toBe(priorBrowser);
    entry.hash = digest('throw new Error("broken");');
    publish(root, manifest);
    refresh();
    expect(renderer).toBe(prior);
    expect(calls).toBe(1);
    const source = 'exports.sourceVariant=()=>"dark";exports.renderPixels=p=>p;';
    fs.writeFileSync(path.join(root, entry.file), source);
    entry.hash = digest(source);
    publish(root, manifest);
    refresh();
    expect(calls).toBe(2);
    expect(renderer.sourceVariant()).toBe('dark');
    expect(errors.length).toBe(2);
    expect(messages).toEqual(['[Modex development] Module loading recovered.']);
    const browserEntry = manifest.modules['theme-icon-runtime'];
    fs.appendFileSync(path.join(root, browserEntry.file), '\n// changed renderer');
    browserEntry.hash = digest(fs.readFileSync(path.join(root, browserEntry.file)));
    publish(root, manifest);
    refresh();
    expect(messages).toHaveLength(2);
    expect(messages[1]).toContain('reload the window or restart');
    expect(calls).toBe(2);
...
test('refuses app bundles, unowned output, corrupt owned output and invalid selections without replacing user files', async () => {
  const root = temporary();
  try {
    const userFile = path.join(root, 'manifest.json');
    fs.writeFileSync(userFile, 'user document');
    await expect(buildDevelopment(root, ['theme-icon'])).rejects.toThrow(
      'Existing development output is invalid',
    );
    expect(fs.readFileSync(userFile, 'utf8')).toBe('user document');
    fs.renameSync(userFile, path.join(root, 'notes.txt'));
    await expect(buildDevelopment(root, ['theme-icon'])).rejects.toThrow('not owned');
    await expect(
      buildDevelopment(path.join(root, 'Stock.app', 'dev'), ['theme-icon']),
    ).rejects.toThrow('app bundle');
    const output = path.join(root, 'output');
    await expect(buildDevelopment(output, ['theme-icon', 'theme-icon'])).rejects.toThrow(
      'duplicates',
    );
...
test('hook hash tracks packaging and native authentication changes while excluding editable and generated sources', () => {
  ...
  try {
    ...
    write('mods/combined/build-mod.mjs', 'bootstrap');
    write('mods/app-tools-auth/bridge.cpp', 'native');
    write('mods/app-tools-auth/bridge.h', 'header');
    const before = computeHookHash(['theme-icon'], root);
    write('mods/app-tools-auth/bridge.h', 'changed header');
    const header = computeHookHash(['theme-icon'], root);
    expect(header).not.toBe(before);
    write('mods/combined/build-mod.mjs', 'changed bootstrap');
    const combined = computeHookHash(['theme-icon'], root);
    expect(combined).not.toBe(header);
    write('mods/theme-icon/render.mjs', 'editable');
    write('mods/combined/dist/bundle.mjs', 'generated');
    expect(computeHookHash(['theme-icon'], root)).toBe(combined);
    write('mods/theme-icon/state.mjs', 'native validation');
    const state = computeHookHash(['theme-icon'], root);
    expect(state).not.toBe(combined);
    ...
    const micro = computeHookHash(['model-spread'], root);
    expect(micro).not.toBe(primary);
...
test('serves fixed renderer ESM URLs without eval, rejects invalid requests, shuts down cleanly', async () => {
  ...
  try {
    ...
    const stop = start({
      ...
      root,
      expected: manifest,
      onIconRenderer: () => calls++,
      intervalMs: 5,
    });
    try {
      expect(responseFor(request('app://-/assets/stock.js'))).toBe(null);
      for (const url of [
        'https://-/assets/modex-development-theme-icon-runtime.mjs',
        'app://fs/assets/modex-development-theme-icon-runtime.mjs',
        'app://user@-/assets/modex-development-theme-icon-runtime.mjs',
        'app://-:9/assets/modex-development-theme-icon-runtime.mjs',
        'app://-/assets/modex-development-theme-icon-runtime.mjs?q=x',
        'app://-/assets/modex-development-theme-icon-runtime.mjs#x',
        'app://-/assets/modex-development-theme-icon-render.mjs',
      ])
        expect(responseFor(request(url)).status).toBe(404);
      expect(responseFor(request(valid.url, 'POST')).status).toBe(405);
      const response = responseFor(valid);
      ...
      const modulePath = path.join(root, 'received.mjs');
      fs.writeFileSync(modulePath, source);
      const loaded = await import(modulePath);
      expect(typeof loaded.Settings).toBe('function');
      expect(typeof loaded.useTheme).toBe('function');
...
test('editor ESM wrapper binds only its fixed model spread dependency', async () => {
  ...
  try {
    fs.writeFileSync(
      ...
      'export const marker="from-model-spread";',
    );
    const source = rendererSource(
      'model-spread-editor',
      'exports.Editor=require("./model-spread.mjs").marker;',
    );
    const file = path.join(root, 'editor.mjs');
    fs.writeFileSync(file, source);
    expect((await import(file)).Editor).toBe('from-model-spread');
    const bad = path.join(root, 'bad.mjs');
    fs.writeFileSync(bad, rendererSource('model-spread-editor', 'require("node:fs");'));
...
test('external pane runtime shares the same drag coordinator as stock hooks', async () => {
  const root = temporary(),
    priorActEnvironment = globalThis.IS_REACT_ACT_ENVIRONMENT;
  let view;
  ...
  try {
    ...
    const store = paneModule.createWorkspaceStore(),
      navigations = [];
    await act(async () => {
      view = TestRenderer.create(
        React.createElement(
          paneModule.Workspace,
          {
            React,
            store,
            route: {
            ...
            Task: ({ task }) =>
              React.createElement('div', { 'data-rendered-task': task.key }, task.title),
          },
    ...
    expect(
      view.root.findByProps({ 'data-modex-drop-preview': 'right' }).findByType('span').children,
    ).toEqual(['Split right']);
    await act(async () => {
      expect(dragModule.drag.drop()).toBe(true);
    });
    ...
    expect(
      view.root.findByProps({ 'data-modex-task-key': 'local:local:b' }).props[
        'data-modex-pane-active'
      ],
    ).toBe('true');
    expect(view.root.findAllByProps({ 'data-modex-drop-preview': 'right' })).toHaveLength(0);
    expect(navigations).toEqual(['/local/b']);
    const bad = path.join(root, 'pane-bad.mjs');
    fs.writeFileSync(bad, rendererSource('task-panes-runtime', 'require("node:fs");'));
    await expect(import(bad)).rejects.toThrow('Unexpected development dependency');
  } finally {
...
test('custom CLI stays packaged while selected renderer modules remain editable', async () => {
  ...
  try {
    ...
    expect(Object.keys(manifest.modules).sort()).toEqual(['task-panes-drag', 'task-panes-runtime']);
    expect(
      inspectDevelopment(root, {
        hookHash: computeHookHash(['task-panes', 'custom-cli']),
        mods: ['task-panes', 'custom-cli'],
        source: manifest.source,
      }),
    ).toEqual(manifest);
    expect(() =>
      inspectDevelopment(root, {
        hookHash: computeHookHash(['task-panes']),
        mods: ['task-panes'],
        source: manifest.source,
      }),
    ).toThrow();
