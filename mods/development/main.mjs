  ...
  const refresh = function () {
    try {
      ...
      const nextSources = Object.fromEntries(
        [...rendererIds]
          .filter((id) => next.modules[id])
          .map((id) => [id, readModule(root, next, id)]),
      );
      const icon = next.modules['theme-icon-render'];
      if (icon && icon.hash !== current?.modules['theme-icon-render']?.hash)
        onIconRenderer(evaluateModule(readModule(root, next, 'theme-icon-render')));
      if (
        current &&
        [...rendererIds].some((id) => next.modules[id]?.hash !== current.modules[id]?.hash)
      )
        log.info(
          '[Modex development] Renderer modules changed; reload the window or restart when ready.',
        );
      current = next;
      sources = nextSources;
      ...
      if (lastError !== error.message)
        log.error('[Modex development] Keeping last good modules:', error.message);
      lastError = error.message;
      return current;
    }
  };
  refresh.readModule = (id) => {
    refresh();
    if (!Object.hasOwn(sources, id)) throw new Error('Development module unavailable');
    return sources[id];
  };
  return refresh;
}
...
export function rendererSource(id, source) {
  ...
  const dependency =
    id === 'model-spread-editor'
      ? `import * as ModelSpread from './model-spread.mjs';\n`
      : id === 'task-panes-runtime'
        ? `import * as TaskPanesDrag from './task-panes-drag.mjs';\n`
        : '';
...
export function responseFor(request) {
  ...
  const match = /^\/assets\/modex-development-([a-z-]+)\.mjs$/.exec(url.pathname),
    id = match?.[1];
  ...
  try {
    ...
    return new Response(rendererSource(id, active.readModule(id)), {
      headers: {
...
export function start({ electron, root, expected, onIconRenderer, intervalMs = 1000 }) {
  const refresh = createRefresh({ root, expected, onIconRenderer });
  let stopped = false;
  refresh();
  active = refresh;
  ...
  const stop = () => {
    ...
    stopped = true;
    clearInterval(timer);
    if (active === refresh) active = undefined;
    electron.app.removeListener('will-quit', stop);
  };
