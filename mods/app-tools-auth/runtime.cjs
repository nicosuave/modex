...
exports.wrap = function wrap(stock, native) {
  return function authorize(socket) {
    ...
    if (!Number.isInteger(fd) || fd < 0)
      return { authorized: false, reason: 'missing-socket-file-descriptor' };
    try {
      const fallback = native.authorizeSocketPeer(fd);
      return fallback?.authorized === true
        ? fallback
        : { authorized: false, reason: fallback?.reason ?? 'modex-peer-rejected' };
    } catch {
