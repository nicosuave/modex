...
export const manifest = JSON.parse(fs.readFileSync(new URL('./compatibility.json', import.meta.url), 'utf8'));
...
const anchor = 'async function mie({callTool:e,listTools:t,pipePath:n,socketPeerAuthorizer:r=Tf()})';
const replacement = 'async function mie({callTool:e,listTools:t,pipePath:n,socketPeerAuthorizer:r=process.platform===`darwin`?require(`./modex-app-tools-auth.cjs`).wrap(Tf(),require(require(`node:path`).join(process.resourcesPath,`native`,`modex-app-tools-auth.node`))):Tf()})';
