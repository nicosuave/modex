...
function validateIdentity({ teamId, bundleId }) {
  if (!/^[A-Z0-9]{10}$/.test(teamId ?? ''))
    throw new Error('A fixed 10-character signing teamId is required');
  if (!/^[A-Za-z0-9][A-Za-z0-9.-]{0,254}$/.test(bundleId ?? ''))
    throw new Error('A fixed bundleId is required');
}

export function inspectNativePrerequisites() {
  if (process.platform !== 'darwin')
    throw new Error('App Tools native authorization requires macOS');
  ...
  const include = ['/opt/homebrew/include/node', '/usr/local/include/node'].find((directory) =>
    fs.existsSync(path.join(directory, 'node_api.h')),
  );
  if (!include)
    throw new Error(
      'Node-API development headers are required in /opt/homebrew/include/node or /usr/local/include/node',
    );
  ...
  try {
    ...
    execFileSync(
      ...
      [
        ...
        prerequisites.sdk,
        '-arch',
        prerequisites.architecture === 'x64' ? 'x86_64' : 'arm64',
        '-I',
