...
socket.on('connect', () => socket.write(JSON.stringify({pid: process.pid, executable: process.execPath})));
socket.on('error', () => process.exit(1));
readline.createInterface({input: process.stdin}).on('line', line => {
  const request = JSON.parse(line);
  if (request.id == null) return;
  const result = request.method === 'initialize'
    ? {protocolVersion: '2024-11-05', capabilities: {}, serverInfo: {name: 'native-auth-fixture', version: '1'}}
    : {tools: []};
  ...
  const reply = () => process.stdout.write(JSON.stringify({jsonrpc: '2.0', id: request.id, result}) + '\n');
