...
suite('native socket authorizer', () => {
  ...
  test('rejects malformed descriptors without coercion', () => {
    for (const value of [undefined, null, '1', -1, 0.5, NaN, Infinity, 2 ** 32, {}, 1n]) {
      expect(addon.authorizeSocketPeer(value)).toEqual({
        authorized: false,
        reason: 'invalid-descriptor',
      });
    }
    expect(addon.authorizeSocketPeer(0, 1)).toEqual({
      authorized: false,
      reason: 'invalid-descriptor',
    });
  });
  ...
  test('rejects real file descriptors that are not peer sockets', () => {
    ...
    try {
      expect(addon.authorizeSocketPeer(fd)).toEqual({
        authorized: false,
        reason: 'peer-token-unavailable',
      });
    } finally {
      fs.closeSync(fd);
    }
  ...
  test('rejects a real connected client signed outside the vendor identity', async () => {
    ...
    try {
      ...
      child = spawn(
        ...
        [
          '-e',
          `require('node:net').connect(${JSON.stringify(socketPath)}).on('error',()=>process.exit(1));`,
        ],
        { stdio: 'ignore' },
      );
      socket = await connection;
      expect(addon.authorizeSocketPeer(socket._handle.fd)).toEqual({
        authorized: false,
        reason: 'peer-signature-invalid',
      });
    } finally {
      socket?.destroy();
      child?.kill();
  ...
  test('requires fixed safe build identities and preserves existing outputs', () => {
    ...
    expect(() =>
      buildNative({ output, teamId: '*', bundleId: 'local.modex.authorizer-test' }),
    ).toThrow('teamId');
    expect(() => buildNative({ output, teamId: 'ABCDE12345', bundleId: 'x" or true' })).toThrow(
      'bundleId',
    );
  });
});
