...
export function verifyNative({ node, codex, signingIdentity, teamId }) {
  ...
  if (!signingTeam || (teamId && teamId !== signingTeam))
    throw new Error('Fixture team must match its Developer ID signing identity');
  teamId = signingTeam;
  ...
  try {
    ...
    for (const binary of [vendorNode, vendorCodex])
      run('/usr/bin/codesign', ['--verify', '--strict', binary]);
    const entitlements = path.join(scratch, 'entitlements.plist');
    fs.writeFileSync(
      entitlements,
      '<?xml version="1.0"?><!DOCTYPE plist PUBLIC "-//Apple//DTD PLIST 1.0//EN" "http://www.apple.com/DTDs/PropertyList-1.0.dtd"><plist version="1.0"><dict><key>com.apple.security.cs.allow-jit</key><true/></dict></plist>',
    );
    ...
    const cases = [
      {
        name: 'accepted signed vendor chain',
        addon: 'accepted',
        expected: { authorized: true, reason: 'authorized' },
      },
      {
        name: 'wrong host bundle',
        addon: 'wrongBundle',
        expected: { authorized: false, reason: 'host-signature-invalid' },
      },
      {
        name: 'wrong host team',
        addon: 'wrongTeam',
        expected: { authorized: false, reason: 'host-signature-invalid' },
      },
      {
        name: 'wrong peer runtime signature',
        addon: 'accepted',
        peerNode: host,
        expected: { authorized: false, reason: 'peer-signature-invalid' },
      },
      {
        name: 'wrong parent identity',
        addon: 'accepted',
        mode: 'wrong-parent',
        expected: { authorized: false, reason: 'parent-signature-invalid' },
      },
      {
        name: 'unrelated host ancestor',
        addon: 'accepted',
        mode: 'wrong-ancestor',
        expected: { authorized: false, reason: 'unrelated-host' },
      },
    ];
    const results = [];
    for (const [index, item] of cases.entries()) {
      ...
      const options = {
        ...item,
        addon: addons[item.addon],
        home,
        node: vendorNode,
        codex: vendorCodex,
        socket: path.join(scratch, `socket-${index}`),
      };
      const output = run(host, [
        path.join(import.meta.dirname, 'native/host-fixture.cjs'),
