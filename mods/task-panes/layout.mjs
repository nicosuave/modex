...
const validTask = task => task && typeof task.key === 'string' && task.key.length > 0
  && typeof task.path === 'string' && typeof task.title === 'string';
const find = (node, id) => !node ? null : node.id === id ? node
  : node.type === 'split' ? find(node.first, id) || find(node.second, id) : null;
const groups = node => !node ? [] : node.type === 'group' ? [node]
  : [...groups(node.first), ...groups(node.second)];
const mapNode = (node, id, change) => !node ? null : node.id === id ? change(node)
  : node.type === 'split' ? { ...node, first: mapNode(node.first, id, change), second: mapNode(node.second, id, change) } : node;
...
function remove(node, key) {
  ...
  if (node.type === 'group') {
    ...
    return tabs.length ? { ...node, tabs, active: node.active === key
      ? tabs[Math.min(index, tabs.length - 1)].key : node.active } : null;
  }
  const first = remove(node.first, key), second = remove(node.second, key);
...
function normalize(state) {
  ...
  return { ...state,
    activeGroup: remaining.some(group => group.id === state.activeGroup) ? state.activeGroup : remaining[0]?.id ?? null,
    maximizedGroup: remaining.some(group => group.id === state.maximizedGroup) ? state.maximizedGroup : null,
  };
}

export function createLayout(task) {
  return validTask(task)
    ? { root: { id: 'group-1', type: 'group', tabs: [{ ...task }], active: task.key }, activeGroup: 'group-1', maximizedGroup: null, nextId: 2 }
...
export function dropTask(state, options = {}) {
  ...
  if (!state || !validTask(task) || !edges.has(edge) || (index !== undefined && (!Number.isInteger(index) || index < 0))) return state;
  if (!state.root) {
    if (targetGroup != null) return state;
    const id = `group-${state.nextId}`;
    return { ...state, root: { id, type: 'group', tabs: [{ ...task }], active: task.key }, activeGroup: id, maximizedGroup: null, nextId: state.nextId + 1 };
  }
  ...
  if (source?.id === targetGroup && edge === 'center') {
    const tabs = index === undefined ? target.tabs : target.tabs.filter(tab => tab.key !== task.key);
    if (index !== undefined) tabs.splice(Math.min(index, tabs.length), 0, reference);
    return { ...state, root: mapNode(state.root, targetGroup, group => ({ ...group, tabs, active: task.key })), activeGroup: targetGroup };
  }
  ...
  root = mapNode(root, targetGroup, group => ({ id: `split-${state.nextId + 1}`, type: 'split',
    axis: edge === 'left' || edge === 'right' ? 'x' : 'y', ratio: 0.5,
    first: edge === 'left' || edge === 'top' ? added : group,
    second: edge === 'left' || edge === 'top' ? group : added,
  }));
  return normalize({ ...state, root, activeGroup: id, maximizedGroup: null, nextId: state.nextId + 2 });
}
...
export function activateTab(state, groupId, key) {
  const group = find(state?.root, groupId);
  if (group?.type !== 'group' || !group.tabs.some(tab => tab.key === key)) return state;
  return { ...state, root: mapNode(state.root, groupId, node => ({ ...node, active: key })), activeGroup: groupId };
}

export function resizeSplit(state, splitId, ratio) {
  if (find(state?.root, splitId)?.type !== 'split' || !Number.isFinite(ratio)) return state;
  return { ...state, root: mapNode(state.root, splitId, node => ({ ...node, ratio: clamp(ratio, 0.05, 0.95) })) };
}

export function toggleMaximize(state, groupId) {
  if (find(state?.root, groupId)?.type !== 'group') return state;
  return { ...state, activeGroup: groupId, maximizedGroup: state.maximizedGroup === groupId ? null : groupId };
}
...
export function layoutRects(state, dimensions = {}, gap = 4) {
  ...
  const minimum = node => node.type === 'group' ? { width: 260, height: 180 } : (() => {
    const a = minimum(node.first), b = minimum(node.second);
    return node.axis === 'x' ? { width: a.width + gap + b.width, height: Math.max(a.height, b.height) }
      : { width: Math.max(a.width, b.width), height: a.height + gap + b.height };
  })();
  const visit = (node, rect) => {
    if (!node) return;
    if (node.type === 'group') { result.groups.push({ id: node.id, group: node, ...rect }); return; }
    const horizontal = node.axis === 'x', dimension = horizontal ? 'width' : 'height';
    const separator = Math.min(gap, rect[dimension]), available = rect[dimension] - separator;
    ...
    const splitter = { ...rect, [horizontal ? 'x' : 'y']: rect[horizontal ? 'x' : 'y'] + firstSize, [dimension]: separator };
    const second = { ...rect, [horizontal ? 'x' : 'y']: splitter[horizontal ? 'x' : 'y'] + separator, [dimension]: secondSize };
    result.splitters.push({ id: node.id, axis: node.axis, ratio: node.ratio, container: { ...rect }, ...splitter });
...
export function hitDropZone(rect, point, edgeFraction = 0.25) {
  if (!rect || !point || ![rect.x, rect.y, rect.width, rect.height, point.x, point.y, edgeFraction].every(Number.isFinite)
