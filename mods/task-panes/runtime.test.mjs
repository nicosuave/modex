...
const nodeMock = (element) =>
  element.type === 'div' ? { getBoundingClientRect: () => bounds } : null;
...
async function setup(initial = null, extra = {}, createNodeMock = nodeMock) {
  ...
  const Task = ({ task }) => {
    ...
    React.useEffect(() => {
      mounted.push(task.key);
      return () => unmounted.push(task.key);
    }, []);
    return React.createElement('input', {
      'data-task-input': task.key,
      value: draft,
      onChange: (event) => setDraft(event.target.value),
      'data-active': pane.active,
      'data-visible': pane.visible,
    });
  };
  const findActive = (node) =>
    !node
      ? null
      : node.type === 'group'
        ? node.id === initial.activeGroup
          ? node.tabs.find((task) => task.key === node.active)
          : null
        : (findActive(node.first) ?? findActive(node.second));
  const activeTask = initial ? findActive(initial.root) : null;
  const props = {
    React,
    route: route(activeTask?.conversationId ?? 'a'),
    navigate: (path) => navigations.push(path),
    Task,
    store,
    coordinator,
    children: React.createElement('div', { 'data-stock': true }, 'Stock'),
    ...extra,
  };
  let view;
  await act(async () => {
    view = TestRenderer.create(React.createElement(Workspace, props), { createNodeMock });
  });
...
test('first accepted drop opens current and dragged tasks while foreign drag remains untouched', async () => {
  ...
  let prevented = false;
  view.root.findByType('div').props.onDragOverCapture({
...
test('moving groups preserves component drafts and same-route rerenders preserve tabs', async () => {
  ...
  await act(async () =>
    view.root
      .findByProps({ 'data-task-input': b.key })
      .props.onChange({ target: { value: 'unsent draft' } }),
  );
  const target = initial.root.first.id;
  await act(async () =>
    store.update((value) => dropTask(value, { task: b, targetGroup: target, edge: 'center' })),
  );
  ...
  await act(async () =>
    view.update(React.createElement(Workspace, { ...props, route: route('a') })),
  );
...
test('ordinary route navigation opens into active group and a repeated existing task stays unique', async () => {
  ...
  await act(async () =>
    view.update(React.createElement(Workspace, { ...props, route: route('b') })),
  );
...
test('remounting a saved workspace reconciles the current route and retains existing tasks', async () => {
  ...
  await act(async () => {
    reopened = TestRenderer.create(
      React.createElement(Workspace, { ...props, route: route('c') }),
...
test('tab keyboard activation and separator keyboard resize affect observable state', async () => {
  ...
  let prevented = false;
  await act(async () =>
    view.root.findAllByProps({ role: 'tab' })[1].props.onKeyDown({
      key: 'Home',
      preventDefault() {
        prevented = true;
      },
  ...
  await act(async () =>
    store.update((value) =>
      dropTask(value, { task: b, targetGroup: value.activeGroup, edge: 'right' }),
    ),
  );
  await act(async () =>
    view.root
      .findByProps({ role: 'separator' })
...
test('composer eligibility excludes hidden or unfocused panes but preserves stock composers', () => {
  ...
  const element = (active, visible, extra = {}) => ({
    closest: () => ({
      getAttribute: (name) =>
        name === 'data-modex-pane-active' ? String(active) : String(visible),
      ...extra,
    }),
  });
...
test('active effects clean up on pane deactivation and stock effects and widths remain available', async () => {
  ...
  function Consumer() {
    const pane = usePane(React),
      width = usePaneWidth(React, 1200);
    useActiveEffect(React, () => {
      calls.push('start');
  ...
  const render = (value) =>
    React.createElement(getPaneContext(React).Provider, { value }, React.createElement(Consumer));
  let view;
  await act(async () => {
    view = TestRenderer.create(render({ active: false, width: 400 }));
  });
  ...
  await act(async () => {
    view = TestRenderer.create(
      React.createElement(
        ...
        null,
        React.createElement(
          getPaneContext(providerReact).Provider,
          { value: { active: true } },
          React.createElement(Consumer, { id: 'active' }),
        ),
        React.createElement(
          getPaneContext(providerReact).Provider,
          { value: { active: false } },
          React.createElement(Consumer, { id: 'inactive' }),
        ),
      ),
    );
  });
...
test('local route references preserve stock local route kind and fallback path', () => {
  expect(
    taskFromRoute({ routeKind: 'local-thread', conversationId: 'abc', hostId: 'machine' }),
  ).toMatchObject({
    key: 'local:machine:abc',
    path: '/local/abc',
    kind: 'local',
    routeKind: 'local-thread',
  });
  expect(taskFromRoute({ ...route('a'), search: '?host=machine' }).path).toBe(
    '/thread/a?host=machine',
  );
...
test('cloud tasks coexist with local tasks and focusing each preserves its source route', async () => {
  ...
  expect(cloud).toEqual({
    ...
    taskId: 'cloud-123',
    key: 'cloud:cloud-123',
    path: '/remote/cloud-123',
    title: 'cloud-123',
  });
  expect(
    taskFromRoute({ ...cloudRoute, pathname: '/remote/cloud-123', search: '?view=details' }).path,
  ).toBe('/remote/cloud-123?view=details');
  ...
  await act(async () =>
    view.update(React.createElement(Workspace, { ...props, route: cloudRoute })),
  );
  expect(store.getSnapshot().root.type).toBe('split');
  await act(async () => findPane(view, a.key).props.onPointerDown());
  expect(navigations.at(-1)).toBe(a.path);
...
test('tab strip drops insert and reorder at tab midpoints instead of splitting', async () => {
  ...
  initial = dropTask(initial, { task: b, targetGroup: initial.activeGroup, edge: 'center' });
  const tab = (task, left) => ({
    getAttribute: () => task.key,
    getBoundingClientRect: () => ({ left, width: 100 }),
  });
  let activeStore;
  const tablist = {
    getAttribute: () => initial.activeGroup,
    querySelectorAll: () =>
      (activeStore?.getSnapshot().root.tabs ?? initial.root.tabs).map((task, index) =>
        tab(task, index * 100),
      ),
  };
  const mock = (element) =>
    element.type === 'div'
      ? { getBoundingClientRect: () => bounds, querySelectorAll: () => [tablist] }
      : null;
  const { view, coordinator, store } = await setup(initial, {}, mock);
  activeStore = store;
  await act(async () => {
    coordinator.start([b]);
    coordinator.move({ x: 10, y: 10 });
  });
  expect(view.root.findByProps({ 'data-modex-drop-preview': 'center' }).props.style.height).toBe(
    40,
  );
  await act(async () => {
    expect(coordinator.drop()).toBe(true);
...
test('own task MIME is consumed during capture before editor drop handlers', async () => {
  ...
  let prevented = 0,
    stopped = 0;
  const event = {
    dataTransfer: { types: ['application/x-modex-task'] },
    clientX: 990,
    clientY: 350,
    preventDefault() {
      prevented++;
  ...
  await act(async () => {
    coordinator.start([b]);
    root.props.onDragOverCapture(event);
    root.props.onDropCapture(event);
  });
...
test('supplied native Button renders all pane controls with stock variants', async () => {
  ...
  expect(
    buttons.every(
      (button) =>
        ['ghostActive', 'ghostSecondary'].includes(button.props.color) &&
        button.props.size === 'compact',
    ),
  ).toBe(true);
...
test.each(['composer-root', 'composer'])(
  'native %s drop targets take precedence over pane preview and consumption',
  async (marker) => {
    ...
    const mock = (element) =>
      element.type === 'div' ? { getBoundingClientRect: () => bounds, ownerDocument } : null;
    const { view, coordinator, store } = await setup(null, {}, mock);
    await act(async () => {
      coordinator.start([b]);
      coordinator.move({ x: 990, y: 350 });
    });
    expect(
    ...
    await act(async () => {
      expect(coordinator.drop()).toBe(false);
    });
    await act(async () => {
      coordinator.start([b]);
      root.props.onDropCapture(event);
...
test('whole-conversation reference wrapper yields to pane placement while nested composer keeps references', async () => {
  ...
  const conversationTarget = {
    querySelector: (selector) => (selector === referenceSelector ? composerTarget : null),
  };
...
test('native pane toolbar and tab components own chrome while task identity survives actions', async () => {
  ...
  const Tab = (props) =>
    React.createElement(
      ...
      React.createElement(
        'button',
        { ...props.tabActivatorProps, onClick: props.onActivate },
        props.title,
      ),
    );
  const { view, store, mounted, unmounted } = await setup(createLayout(a), {
    Toolbar,
    ToolbarActions,
    Tab,
  });
...
for (const kind of ['local', 'cloud'])
  test(`sidebar selection focuses existing ${kind} task without moving it or collapsing split groups`, async () => {
    const selected =
      kind === 'local' ? a : taskFromRoute({ routeKind: 'remote-thread', taskId: 'cloud-keep' });
    let initial = createLayout(selected);
    initial = dropTask(initial, { task: b, targetGroup: initial.activeGroup, edge: 'right' });
    initial = { ...initial, root: { ...initial.root, ratio: 0.63 } };
    const { view, store, props, mounted, unmounted } = await setup(initial);
    const targetRoute =
      kind === 'local' ? route('a') : { routeKind: 'remote-thread', taskId: 'cloud-keep' };
    for (const nextRoute of [targetRoute, route('b'), targetRoute, route('b')]) {
      await act(async () =>
        view.update(React.createElement(Workspace, { ...props, route: nextRoute })),
      );
      expect(store.getSnapshot().root.type).toBe('split');
    ...
    expect(mounted).toEqual([selected.key, b.key]);
    expect(unmounted).toEqual([]);
    await act(async () => view.unmount());
  });
...
test('new sidebar tasks add to focused pane and existing hidden tabs focus in place', async () => {
  ...
  let initial = createLayout(a);
  initial = dropTask(initial, { task: c, targetGroup: initial.activeGroup, edge: 'center' });
  initial = dropTask(initial, { task: b, targetGroup: initial.activeGroup, edge: 'right' });
  const { view, store, props } = await setup(initial);
  await act(async () =>
    view.update(React.createElement(Workspace, { ...props, route: route('d') })),
  );
  expect(store.getSnapshot().root.second.tabs.map((task) => task.key)).toEqual([
    b.key,
    'local:local:d',
  ]);
  ...
  await act(async () => {
    reopened = TestRenderer.create(
      React.createElement(Workspace, { ...props, route: route('b') }),
      { createNodeMock: nodeMock },
    );
  });
  expect(store.getSnapshot().root.type).toBe('split');
