...
export function getRenderer(React, loaders = defaultLoaders) {
  ...
  const components = Object.fromEntries(
    Object.entries(loaders).map(([kind, load]) => [kind, React.lazy(load)]),
  );
  function Task({ task }) {
    ...
    return React.createElement(
      React.Suspense,
      {
        fallback: React.createElement(
          'div',
          { role: 'status', style: { padding: 16 } },
          'Loading task…',
        ),
      },
      Component
        ? React.createElement(Component, { task })
        : React.createElement(
            'div',
            { role: 'alert' },
            'This task type cannot be opened in a pane.',
          ),
    );
  }
