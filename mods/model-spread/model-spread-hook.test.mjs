...
const choices = ['a', 'b', 'c', 'd'].map((model) => ({
  model,
  reasoningEffort: 'high',
  id: `${model}:high`,
}));
function Probe(props) {
  useComposer(React, props);
  return null;
}
function turn(root, direction = 1) {
  root.dispatchEvent(new CustomEvent(EVENT, { detail: { direction } }));
}
async function harness(initial = {}) {
  testStorage.fail = false;
  store().save({ ...EMPTY, micro: true });
  const root = new EventTarget(),
    trigger = { current: { closest: () => root } },
    calls = [];
  let props = {
    trigger,
    choices,
  ...
  return {
    ...
    async turn(direction = 1) {
      await act(async () => turn(root, direction));
    },
    async close() {
...
test('attaches after initial loading when the stable trigger ref receives a root', async () => {
  const trigger = { current: null },
    h = await harness({ trigger, enabled: false });
  try {
    await h.turn();
...
test('rapid detents advance before React renders and acknowledge intermediate updates', async () => {
  ...
  try {
    ...
    await h.update({ model: 'b' });
    await h.turn();
    assert.deepEqual(h.calls, ['b', 'c', 'd']);
    await h.update({ model: 'd' });
    await h.turn();
    assert.deepEqual(h.calls, ['b', 'c', 'd']);
    await h.turn(-1);
    assert.deepEqual(h.calls, ['b', 'c', 'd', 'c']);
  } finally {
    await h.close();
  }
});
...
test('a rejected selection does not leave an optimistic cursor behind', async () => {
  ...
  const h = await harness({
    select: (choice) => {
      calls.push(choice.model);
      return Promise.resolve(fail ? false : undefined);
    },
  });
  try {
    await h.turn();
...
test('a rapid reversal back to the original pair is acknowledged before mouse selection', async () => {
  ...
  try {
    ...
    await h.update({ model: 'b' });
    await h.turn();
    assert.deepEqual(h.calls, ['b', 'a', 'c']);
  } finally {
    await h.close();
  }
});
...
test('mode and composer disable suppress events and reset unacknowledged navigation', async () => {
  ...
  try {
    ...
    await h.update({ enabled: true });
    await h.turn();
    assert.deepEqual(h.calls, ['b', 'b', 'b']);
  } finally {
    await h.close();
  }
});
...
test('moving roots and unmounting detach the old event listener', async () => {
  ...
  await h.turn();
  assert.deepEqual(h.calls, []);
  await act(async () => turn(replacement));
  assert.deepEqual(h.calls, ['b']);
  await h.close();
  turn(replacement);
  assert.deepEqual(h.calls, ['b']);
