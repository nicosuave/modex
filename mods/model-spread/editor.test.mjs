    ...
    open &&
      h(
        ...
        h(
          'button',
          {
            'aria-label': size === 'wide' ? 'Close editor' : 'Close confirmation',
...
const Native = {
  ...
  Section: Container,
  Footer: Container,
  Heading: ({ title, subtitle }) => h('header', null, title, subtitle),
  Title: ({ children }) => h('h2', null, children),
  Description: ({ children }) => h('p', null, children),
...
function Picker({ slot, onChange }) {
  return h(
    ...
    h(
      ...
      models.map((model) =>
        h('option', { key: model.model, value: model.model }, model.displayName),
      ),
    ),
    h(
      'select',
      {
        'aria-label': 'Slot effort',
        value: slot.reasoningEffort,
        onChange: (event) => onChange({ ...slot, reasoningEffort: event.target.value }),
      },
      models
        .find((model) => model.model === slot.model)
        ?.supportedReasoningEfforts.map(({ reasoningEffort }) =>
          h('option', { key: reasoningEffort, value: reasoningEffort }, reasoningEffort),
        ),
    ),
  );
}
...
async function harness(slots = initial) {
  ...
  await act(async () => {
    renderer = TestRenderer.create(
      h(Editor, {
        ...
        models,
        defaults,
        trigger: h('button', null, 'Configure'),
      }),
    );
  });
  const button = (label) =>
    renderer.root
      ...
      .find((node) => node.props['aria-label'] === label || text(node) === label);
  const groups = () =>
    renderer.root.findAll((node) => node.type === 'div' && node.props.role === 'group');
  const indicators = () =>
    groups().flatMap((node, index) => [
  ...
  return {
    ...
    async click(label) {
      ...
      assert.notEqual(node.props.disabled, true, `Button ${label} is enabled`);
      await act(async () => node.props.onClick());
    },
    async choose(label, value, index = 0) {
      const group = groups()[index];
      assert.ok(group, `Slot ${index + 1} exists`);
      const select = group
    ...
    async key(index, key) {
      const node = button(`Reorder slot ${index + 1}`);
      assert.ok(node);
      let prevented = false;
      await act(async () =>
    ...
    async drag(from, to, edge = 'before', { cancel = false } = {}) {
      const handle = button(`Reorder slot ${from + 1}`),
        data = new Map();
      ...
      const event = {
        dataTransfer,
        clientY: rect.top + (edge === 'after' ? 33 : 11),
      ...
      const expected =
        edge === 'after' && to < groups().length - 1
          ? { index: to + 1, edge: 'before' }
          : { index: to, edge };
      assert.deepEqual(
        indicators(),
        [expected],
        'one insertion divider appears at the hovered boundary',
      );
    ...
    rows() {
      return groups().map((node) => node.props['aria-label']);
    },
    pickerCounts() {
      return groups().map((node) => node.findAllByType('select').length);
    },
    alerts() {
      return renderer.root.findAll((node) => node.props.role === 'alert').map(text);
    },
    async close() {
      storage.fail = false;
...
test('drag reorders inline slots and editing afterward targets the moved row', async () => {
  ...
  try {
    ...
    await editor.click('Done');
    assert.deepEqual(store().get().slots, [
      { model: 'a', reasoningEffort: 'high' },
      initial[1],
      { model: 'c', reasoningEffort: 'max' },
    ]);
  } finally {
    await editor.close();
  }
...
test('drag insertion boundaries beside the source are no-ops and canceled drag preserves order', async () => {
  const slots = [...initial, { model: 'c', reasoningEffort: 'max' }],
    editor = await harness(slots);
  try {
    await editor.click('Configure');
  ...
  try {
    ...
    assert.equal(
      editor.renderer.root.findAll(
        (node) => node.type === 'section' && node.props.role === 'dialog',
      ).length,
      2,
    );
...
test('unavailable saved slots remain visible and retain their original model pair', async () => {
  const unavailable = { model: 'retired-model', reasoningEffort: 'ultra' },
    editor = await harness([unavailable]);
  try {
    await editor.click('Configure');
    ...
    assert.ok(
      editor.renderer.root
        .findAllByType('span')
        .some((node) => text(node).includes('Unavailable on this host')),
    );
    await editor.click('Done');
