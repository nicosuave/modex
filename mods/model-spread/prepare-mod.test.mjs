...
async function fixture(fn) {
  ...
  try {
    const source=path.join(directory,'Test.app'),backup=path.join(directory,'original.zip');
...
test('prepare arguments require explicit output and do not silently accept unknown flags',()=>{
  assert.deepEqual(parseArgs(['--output','/tmp/Mod.app','--check']),{source:'/Applications/ChatGPT.app',output:'/tmp/Mod.app',check:true});
...
test('existing backup CRC and source ASAR/Info.plist fingerprints are verified without writing',async()=>fixture(async({source,backup})=>{
  const before=await hashFile(backup),report=await verifyBackup(backup,source);
  assert.equal(report.zipCRCVerified,true);
  assert.equal(report.sourceAsarHash,await hashFile(path.join(source,'Contents/Resources/app.asar')));
  assert.equal(await hashFile(backup),before);
}));

test('a different source archive or metadata rejects backup reuse',async()=>fixture(async({source,backup})=>{
...
test('corrupt ZIP is rejected before considering its fingerprints',async()=>fixture(async({source,backup})=>{
  fs.truncateSync(backup,fs.statSync(backup).size-50);
  await assert.rejects(verifyBackup(backup,source),/unzip exited/);
}));

test('prepare refuses an existing output before any source or backup operation',async()=>fixture(async({source})=>{
  await assert.rejects(main(['--output',source,'--source','/does-not-exist.app']),/ENOENT|already exists/);
