...
if(args.length===1&&args[0]==='--help') {
  ...
  try {
    ...
    const originals=Object.fromEntries([...bundles].map(([name,bytes])=>[name,bytes.toString('utf8')]));
    const patched=transform(originals);
    if(JSON.stringify(patched)!==JSON.stringify(transform(originals)))throw Error('Transform output is not deterministic');
    ...
    for(const [name,content] of Object.entries(patched)) {
      ...
      for(const item of parser.scan(content).imports) {
        ...
        const generated=target.startsWith('webview/assets/')&&Object.hasOwn(patched,target.slice('webview/assets/'.length));
        if(!generated&&!entryFor(archive,target))throw Error(`Missing import ${item.path} in ${name}`);
      }
    }
    ...
    console.log(JSON.stringify({verified:true,version:manifest.version,build:manifest.build,bundles:bundles.size,overlayFiles:Object.keys(patched).length,deterministic:true,syntaxAndImportPaths:true,tests:'passed with native HOME regression',appModified:false,launched:false},null,2));
