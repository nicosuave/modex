...
const directories = [];
const temporary = () => {
  const value = fs.mkdtempSync(path.join(os.tmpdir(), 'modex-cli-test-'));
...
test('default file is optional; an explicit configuration path is required and absolute', () => {
  ...
  for (const explicit of ['', 'relative.json', path.join(userData, 'missing.json')])
    expect(() =>
      loadConfiguration({ userData, env: { MODEX_CUSTOM_CLI_CONFIG: explicit } }),
    ).toThrow('Custom CLI:');
  fs.writeFileSync(
    path.join(userData, 'modex-custom-cli.json'),
    JSON.stringify({ configOverrides: ['model="example"'] }),
  );
  ...
  expect(
    loadConfiguration({ userData, env: { MODEX_CUSTOM_CLI_CONFIG: external } }).configOverrides,
  ).toEqual(['model="external"']);
});
...
test('disabled and empty configurations are inactive; executable paths require executable files', () => {
  ...
  expect(
    validateConfiguration({
      enabled: false,
      executablePath: '/missing/file',
      configOverrides: ['model="example"'],
    }),
  ).toBeNull();
  ...
  expect(() => validateConfiguration({ executablePath: path.dirname(executable) })).toThrow(
    'executable file',
  );
});
...
test('schema errors and filesystem failures never include configuration contents', () => {
  ...
  const userData = temporary(),
    file = path.join(userData, 'modex-custom-cli.json');
  fs.writeFileSync(file, '{"' + secret + '":');
  expect(() => loadConfiguration({ userData, env: {} })).toThrow(
  ...
  expect(() => loadConfiguration({ userData, env: {}, fileSystem: broken })).toThrow(
...
test('TOML values retain exact bytes and argv boundaries; local overrides follow stock overrides', () => {
  const values = [
    ...
    'model_providers.example.base_url="https://example.invalid?a=b"',
  ...
  expect(modified.env).toBe(launch.env);
  expect(modified.cwd).toBe(launch.cwd);
  expect(applyLaunch(launch, { kind: 'local' }, null)).toBe(launch);
  for (const kind of ['ssh', 'wsl', 'remote-control', 'cloud', 'websocket'])
    expect(applyLaunch(launch, { kind }, config)).toBe(launch);
...
test('initialization uses final app userData once and aligns the stock native CLI environment', async () => {
  ...
  const userData = temporary(),
    executable = path.join(userData, 'custom cli');
  fs.writeFileSync(executable, '#!/bin/sh\nexit 0\n', { mode: 0o700 });
  const file = path.join(userData, 'modex-custom-cli.json');
  fs.writeFileSync(
    file,
    JSON.stringify({ executablePath: executable, configOverrides: ['model="example"'] }),
  );
  ...
  expect(env).toEqual({ CODEX_CLI_PATH: executable, OTHER: 'retained' });
  ...
  for (const config of [{ enabled: false }, {}]) {
    ...
    fresh.initialize(app, untouched);
    expect(untouched).toEqual({ CODEX_CLI_PATH: '/existing' });
    expect(fresh.isActive()).toBe(false);
  }
});

test.skipIf(!process.env.CUSTOM_CLI_EXECUTABLE)(
  'bundled app-server reads the custom override after app-supplied settings',
  async () => {
    const config = validateConfiguration({ configOverrides: ['model="modex-override-check"'] });
    const launch = applyLaunch(
      {
        executablePath: process.env.CUSTOM_CLI_EXECUTABLE,
        args: ['app-server', '--stdio', '-c', 'model="stock-check"'],
        env: { ...process.env, CODEX_HOME: temporary() },
      },
      { kind: 'local' },
      config,
    );
    const child = Bun.spawn([launch.executablePath, ...launch.args], {
      env: launch.env,
      stdin: 'pipe',
      stdout: 'pipe',
      stderr: 'ignore',
    ...
    function request(method, params) {
      ...
      return new Promise((resolve, reject) => {
        const timer = setTimeout(() => {
          pending.delete(id);
          reject(Error('App-server request timed out'));
        }, 10000);
        pending.set(id, (message) => {
          clearTimeout(timer);
          pending.delete(id);
          message.error ? reject(Error('App-server rejected request')) : resolve(message.result);
        });
    ...
    try {
      await request('initialize', {
        clientInfo: { name: 'modex_custom_cli_test', version: '1' },
