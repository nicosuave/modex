...
export function parseArgs(args) {
  ...
  for (let i = 0; i < rest.length; i++) {
    ...
    if ((flag === '--json' && command === 'status') || (flag === '--watch' && command === 'dev')) {
      result.args.push(flag);
      continue;
    }
    const valueFlags = {
    ...
    if (!valueFlags.includes(flag) || !rest[i + 1] || rest[i + 1].startsWith('--'))
      throw Error(`Unknown or incomplete argument: ${flag}`);
    const value = rest[++i];
    if (flag === '--mods') {
      result.mods = selectMods(value.split(','));
      result.explicitSelection = true;
...
export async function main(args = process.argv.slice(2)) {
  ...
  if (options.command === 'status') {
    ...
    const app =
      appIndex < 0
        ? path.join(os.homedir(), 'Applications', 'Modex.app')
        : options.args[appIndex + 1];
    ...
    if (options.args.includes('--json')) console.log(JSON.stringify(status, null, 2));
      ...
      if (receipt) {
        ...
        if (status.development) {
          ...
          for (const module of status.development.modules ?? [])
            if (module.changedSincePackaging) console.log(`Updated since packaging: ${module.id}`);
          console.log(
            'External module status does not establish which revision a running window has loaded.',
          );
        }
      } else console.log('Legacy build: no source revision or transform hashes recorded.');
      console.log(
        'Receipt checks cover recorded files, not the app signature or external development modules.',
      );
    }
    if (
      status.receiptStatus === 'mismatch' ||
      (status.development && status.development.status !== 'ready')
    )
      process.exitCode = 1;
    return status;
  }
  if (options.command === 'dev') {
    ...
    if (options.args.includes('--watch')) {
      ...
      console.log(
        'Watching mod source. Icon pixels reload live; renderer changes require a window reload or restart.',
      );
    }
    return manifest;
  }
  const entry = await import(`./mods/combined/${options.command}-mod.mjs`);
  return entry.main(options.args, {
    mods: options.mods,
