...
const help = `Usage: bun package-app.mjs --source /Applications/ChatGPT.app --overlay DIRECTORY --output /absolute/path/Model-Spread.app --app-name NAME --bundle-id local.codex.mod-name [--inspect] [--unsigned]
...
export function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (['--source', '--overlay', '--output', '--app-name', '--bundle-id'].includes(name) && args[i + 1] && !args[i + 1].startsWith('--')) options[name.slice(2)] = args[++i];
...
export function main(args=process.argv.slice(2)) {
  ...
  const signer=options.unsigned?null:signingIdentity();
...
if(import.meta.main)try { main(); } catch (error) {
