...
import { readArchive, readOverlay, rewriteArchive, readEntry, entryFor, integrityForBuffer } from './asar.mjs';
...
function command(name, args) {
  return execFileSync(name, args, { encoding: 'utf8', maxBuffer: 16 * 1024 * 1024, stdio: ['ignore', 'pipe', 'pipe'] });
}
export function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (['--source', '--overlay', '--output', '--app-name', '--bundle-id','--dev-root'].includes(name) && args[i + 1] && !args[i + 1].startsWith('--')) options[name.slice(2)] = args[++i];
...
export async function main(args=process.argv.slice(2)) {
  ...
  for (const key of ['source', 'overlay', 'output']) if (!options[key]) throw new Error(`Missing --${key}\n${help}`);
  if (process.platform !== 'darwin') throw new Error('App packaging and codesigning require macOS');
  if (!path.isAbsolute(options.output) || !options.output.endsWith('.app')) throw new Error('--output must be an absolute .app path');
  ...
  if (contained(source, output) || contained(output, source) || contained(overlay, output) || contained(output, overlay)) throw new Error('Source, overlay and output must be separate directories');
  if (contained('/Applications', output) || contained(path.join(os.homedir(), 'Applications'), output)) throw new Error('Output must be a staging location outside Applications');
  ...
  if (info.ElectronAsarIntegrity?.['Resources/app.asar']?.hash !== archive.headerHash) throw new Error('Stock ASAR header does not match its Info.plist integrity hash');
  const replacements = readOverlay(overlay);
  if (!replacements.size) throw new Error('Overlay is empty');
  const metadata=replacements.has(metadataPath)?JSON.parse(replacements.get(metadataPath).toString()):null;
  ...
  if(metadata?.receipt)metadata.receipt.transforms.push(...recordChanges('app-tools-auth','app-tools authentication',beforeRepair,replacements));
  if(options['dev-root']) {
    if(!metadata?.receipt)throw Error('Development packaging requires a build receipt; use modex prepare');
    const {applyDevelopment,readDevelopmentProtocol,protocolPath}=await import('../mods/development/patch.mjs');
    ...
    metadata.receipt.development=await applyDevelopment(replacements,options['dev-root'],metadata.mods,protocolOriginal);
    metadata.receipt.transforms.push(...recordChanges('development','development module bootstrap',beforeDevelopment,replacements));
  }
  if(metadata?.receipt)replacements.set(metadataPath,Buffer.from(JSON.stringify(metadataFor(metadata.mods,metadata.receipt),null,2)+'\n'));
  checkNativeBuild();
  ...
  const plan = { source, output, version: info.CFBundleShortVersionString, build: info.CFBundleVersion, sourceHeaderHash: archive.headerHash, files: [...replacements.keys()], signing: options.unsigned ? 'unsigned inspection only' : signer.name };
  ...
  command('/usr/bin/codesign',['--force','--sign',signer.hash,'--options','runtime',nativeOutput]);
  ...
  for (const [filename, bytes] of replacements) {
    ...
    if (JSON.stringify(entryFor(generated, filename).integrity) !== JSON.stringify(integrityForBuffer(actual))) throw new Error(`Packaged integrity differs: ${filename}`);
  }
  ...
  command('/usr/bin/plutil', ['-replace', 'ElectronAsarIntegrity', '-json', JSON.stringify({ 'Resources/app.asar': { algorithm: 'SHA256', hash: result.headerHash } }), plist]);
  const signing=options.unsigned?null:signApp(output,{identity:signer.hash,original:source,appName:options['app-name'],bundleID:options['bundle-id']});
  // Check the original is still sealed and its ASAR header is unchanged.
  if (readArchive(sourceAsar).headerHash !== archive.headerHash) throw new Error('Source archive changed during packaging');
  command('/usr/bin/codesign', ['--verify', '--deep', '--strict', source]);
  console.log(JSON.stringify({ ...plan, ...result, removedEntitlements: signing?.removedEntitlements??[], addedEntitlements: signing?.addedEntitlements??[], designatedRequirement: signing?.requirement??null, sourceSignatureVerified: true, stagedSignatureVerified: !options.unsigned, launched: false }, null, 2));
}
