...
import { resolveBundlePaths, canonicalBundles, actualBundles } from './current-source.mjs';
...
export function parseArgs(args) {
  ...
  for (let i = 0; i < args.length; i++) {
    const name = args[i];
    if (['--inspect', '--unsigned', '--help', '--current-source'].includes(name))
...
export async function main(args = process.argv.slice(2)) {
  ...
  const { mainPath: canonicalMain } = await import('../mods/app-tools-auth/patch.mjs');
  const currentSource = options['current-source'] === true;
  const mainPath = currentSource
    ? resolveBundlePaths(archive, [canonicalMain]).get(canonicalMain)
    : canonicalMain;
  ...
  if (options['dev-root']) {
    ...
    const devPaths = currentSource
      ? resolveBundlePaths(archive, [canonicalMain, protocolPath])
      : new Map();
    const protocolOriginal = readDevelopmentProtocol(source, { currentSource });
    beforeDevelopment.set(
      devPaths.get(protocolPath) ?? protocolPath,
      readEntry(archive, devPaths.get(protocolPath) ?? protocolPath),
    );
    ...
    for (const [name, value] of Object.entries(
      actualBundles(Object.fromEntries(devReplacements), devPaths),
    ))
      replacements.set(name, Buffer.from(value));
    metadata.receipt.transforms.push(
      ...recordChanges(
        'development',
