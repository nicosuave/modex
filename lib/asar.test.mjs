...
function fixture(directory) {
  ...
  const header = {
    files: {
      'first.js': {
        size: one.length,
        offset: '0',
        integrity: integrityForBuffer(one),
        executable: true,
      },
      assets: {
        files: {
          'binary.bin': {
            size: two.length,
            offset: String(one.length),
            integrity: integrityForBuffer(two),
          },
        },
      },
      native: {
        files: {
          'addon.node': {
            size: 3,
            unpacked: true,
            integrity: integrityForBuffer(Buffer.from('xyz')),
          },
        },
        unpacked: true,
      },
  ...
  fs.writeFileSync(
    filename,
    Buffer.concat([prefix, json, Buffer.alloc(picklePayloadLength - 4 - json.length), one, two]),
  );
...
test('roundtrip keeps packed bytes, links, unpacked metadata and source bytes', () =>
  temporary((directory) => {
...
test('changed lengths, additions, empty files and multiple integrity blocks remain readable', () =>
  temporary((directory) => {
    const source = fixture(directory);
    ...
    assert.deepEqual(
      entryFor(archive, 'native/addon.node'),
      source.header.files.native.files['addon.node'],
    );
  }));

test('rejects traversal, unpacked replacement, directory replacement and self mutation', () =>
  temporary((directory) => {
    ...
    for (const name of [
      ...
      assert.throws(() =>
        rewriteArchive(filename, output, new Map([[name, Buffer.from('invalid')]])),
      );
...
test('overlay recursively reads files and rejects symlinks', () =>
  temporary((directory) => {
...
test('rejects truncated input before publishing replacement archive', () =>
  temporary((directory) => {
    const { filename } = fixture(directory);
