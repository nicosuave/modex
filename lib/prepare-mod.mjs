...
import {metadataPath,metadataFor,readInstalledMods,assertModSelection} from './mod-metadata.mjs';
...
export function parseArgs(args) {
  ...
  for(let index=0;index<args.length;index++) {
    ...
    else if(['--source','--output','--backup','--identity-from','--dev-root'].includes(flag)&&args[index+1]&&!args[index+1].startsWith('--')) {
...
export function compatibilityResourcePath(name) {
  if(typeof name!=='string'||name.includes('\\')||name.includes('\0')||name.split('/').some(part=>!part||part==='.'||part==='..'))throw Error(`Invalid compatibility filename: ${name}`);
  return name;
}
...
export function inspectCompatibility(source,manifest) {
  ...
  for(const [name,expected]of Object.entries(manifest.resources??{})) {
    ...
    if(!contained(resourceRoot,filename))throw Error(`Compatibility resource escapes app resources: ${name}`);
    const bytes=fs.readFileSync(filename);
    if(crypto.createHash('sha256').update(bytes).digest('hex')!==expected)throw Error(`Unsupported resource ${name}; original app unchanged`);
  }
...
export function validateIdentityContinuity(bundleId,requirement,signerName) {
  ...
  const installedTeam=requirement.match(/certificate leaf\[subject\.OU\] = (?:"([A-Z0-9]+)"|([A-Z0-9]+))(?:\s|$)/);
  if(!team||!requirement.includes(`identifier "${bundleId}"`)||(installedTeam?.[1]??installedTeam?.[2])!==team)throw Error('Selected signing certificate does not preserve the installed mod identity and team');
}

export async function prepareMain(args=process.argv.slice(2),{modDirectory,overlayAtRoot=false,help,manifest:providedManifest,selectedMods,explicitSelection=true,legacyMarkers={},buildOverlay}={}) {
  ...
  let installedMods=null,removedMods=[];
  ...
  if(options['identity-from']) {
    ...
    const installedInfo=JSON.parse(command('/usr/bin/plutil',['-convert','json','-o','-',path.join(installed,'Contents/Info.plist')]));
    ...
    installedMods=readInstalledMods(readArchive(path.join(installed,'Contents/Resources/app.asar')),legacyMarkers);
    removedMods=assertModSelection(installedMods,mods,explicitSelection);
  }
  validateAppIdentity(mod.appName,mod.bundleId);
  const manifest=providedManifest??JSON.parse(fs.readFileSync(path.join(modDirectory,'compatibility.json'),'utf8'));
  ...
  if(options['dev-root']) {
    ...
    inspectDevelopmentForBuild(options['dev-root'],mods,source);
  }
  verifyRepair(source);
  checkNativeBuild();
  ...
  if(options.check) {
    console.log(JSON.stringify({checkOnly:true,mods,installedMods,removedMods,source,output,development:options['dev-root']??null,bundleId:mod.bundleId,expectedRequirement,signer:signer.name,backup,version:info.CFBundleShortVersionString,build:info.CFBundleVersion,sourceHeaderHash:archive.headerHash,compatibleBundles:bundles.size,backupExists:!!evidence,wouldCreateBackup:!evidence,...evidence,launched:false},null,2));
    return;
  }
  if(!evidence) {
  ...
  try {
    const input=path.join(temporary,'bundles'),overlay=path.join(temporary,overlayAtRoot?'overlay':'overlay/webview/assets');
    fs.mkdirSync(input);
    ...
    let transforms;
    if(buildOverlay)transforms=await buildOverlay(input,overlay);
    else await run(process.execPath,[path.join(modDirectory,'build-mod.mjs'),input,overlay],{inherit:true});
    if(!transforms)transforms=recordChanges(mods[0],`${mods[0]} adapters`,new Map([...bundles].map(([name,bytes])=>[compatibilityEntryPath(name),bytes])),readOverlay(path.join(temporary,'overlay')));
    const receipt={source:{version:info.CFBundleShortVersionString,build:info.CFBundleVersion,asarHash:evidence.sourceAsarHash},build:buildIdentity(),transforms};
    fs.writeFileSync(path.join(temporary,'overlay',metadataPath),JSON.stringify(metadataFor(mods,receipt),null,2)+'\n');
    ...
    await run(process.execPath,[path.join(here,'package-app.mjs'),'--source',source,'--overlay',path.join(temporary,'overlay'),'--output',output,'--app-name',mod.appName,'--bundle-id',mod.bundleId,...(options['dev-root']?['--dev-root',options['dev-root']]:[])],{inherit:true});
    if(expectedRequirement&&designatedRequirement(output)!==expectedRequirement)throw Error('Packaged designated requirement differs from installed app; do not adopt this staged copy');
    if(JSON.stringify(readInstalledMods(readArchive(path.join(output,'Contents/Resources/app.asar'))))!==JSON.stringify(mods))throw Error('Packaged mod selection differs from requested mods');
    ...
    console.log(JSON.stringify({mods,installedMods,removedMods,source,output,bundleId:mod.bundleId,expectedRequirement,signer:signer.name,backup,...evidence,secondCopyCreated:true,originalModified:false,launched:false},null,2));
