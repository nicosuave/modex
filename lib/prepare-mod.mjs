...
import {signingIdentity,validateAppIdentity} from './sign-app.mjs';
...
export async function prepareMain(args=process.argv.slice(2),{modDirectory,overlayAtRoot=false,help}={}) {
  ...
  const manifest=JSON.parse(fs.readFileSync(path.join(modDirectory,'compatibility.json'),'utf8'));
  ...
  try {
    ...
    await run(process.execPath,[path.join(modDirectory,'build-mod.mjs'),input,overlay],{inherit:true});
