...
export function parseArgs(args) {
  ...
  for(let index=0;index<args.length;index++) {
    ...
    else if(['--source','--output','--backup','--identity-from'].includes(flag)&&args[index+1]&&!args[index+1].startsWith('--')) {
...
export async function prepareMain(args=process.argv.slice(2),{modDirectory,overlayAtRoot=false,help,manifest:providedManifest,selectedMods,explicitSelection=true,legacyMarkers={},buildOverlay}={}) {
  ...
  if(options.check) {
    console.log(JSON.stringify({checkOnly:true,mods,installedMods,removedMods,source,output,bundleId:mod.bundleId,expectedRequirement,signer:signer.name,backup,version:info.CFBundleShortVersionString,build:info.CFBundleVersion,sourceHeaderHash:archive.headerHash,compatibleBundles:bundles.size,backupExists:!!evidence,wouldCreateBackup:!evidence,...evidence,launched:false},null,2));
    return;
  }
  if(!evidence) {
  ...
  try {
    ...
    if(buildOverlay)await buildOverlay(input,overlay);
    else await run(process.execPath,[path.join(modDirectory,'build-mod.mjs'),input,overlay],{inherit:true});
    fs.writeFileSync(path.join(temporary,'overlay',metadataPath),JSON.stringify(metadataFor(mods),null,2)+'\n');
    ...
    await run(process.execPath,[path.join(here,'package-app.mjs'),'--source',source,'--overlay',path.join(temporary,'overlay'),'--output',output,'--app-name',mod.appName,'--bundle-id',mod.bundleId],{inherit:true});
