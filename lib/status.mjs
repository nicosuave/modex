...
function developmentStatus(receipt) {
  ...
  if(!descriptor.hookHash)return {status:'unavailable',modules:[],error:'Packaged receipt has no development hook hash'};
  try {
  ...
  try {
    const manifest=inspectDevelopment(descriptor.root,{hookHash:descriptor.hookHash,mods:descriptor.mods,source:receipt.source});
    const modules=Object.entries(manifest.modules).map(([id,entry])=>({id,hash:entry.hash,
      changedSincePackaging:descriptor.moduleHashes?.[id]===undefined?null:descriptor.moduleHashes[id]!==entry.hash}));
...
export function readStatus(appPath,{legacyMarkers={}}={}) {
  if(typeof appPath!=='string'||!path.isAbsolute(appPath))throw Error('Status requires an absolute app path');
  const archive=readArchive(path.join(appPath,'Contents','Resources','app.asar'));
  const metadata=readInstalledMetadata(archive);
  const mods=metadata?.mods??Object.entries(legacyMarkers).filter(([,marker])=>entryFor(archive,marker)).map(([id])=>id);
  const expected=new Map();
  for(const transform of metadata?.receipt?.transforms??[])expected.set(transform.path,transform.afterHash);
  ...
  return {appPath:path.resolve(appPath),metadata,mods,
    receiptStatus:!metadata?.receipt?'unavailable':files.every(file=>file.status==='match')?'verified':'mismatch',files,
