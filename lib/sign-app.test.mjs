...
test('different app builds retain the same certificate requirement, normal profile and native icon',{skip:!(process.env.CODEX_MODS_SIGN_IDENTITY??process.env.MODEL_SPREAD_SIGN_IDENTITY),timeout:60000},()=>{
 ...
 try {
  ...
  const renamedSignature=signApp(renamed,{home,original:source,appName:'Renamed Mod',bundleID});
  assert.equal(renamedSignature.requirement,one.requirement);
  execFileSync('/usr/bin/codesign',['--verify','--deep','--strict','-R','='+one.requirement,renamed]);
  const renamedInfo=JSON.parse(command(['-convert','json','-o','-',path.join(renamed,'Contents/Info.plist')]));
  ...
  assert.equal(renamedInfo.LSEnvironment.CODEX_ELECTRON_USER_DATA_PATH,path.join(home,'Library/Application Support/Codex'));
