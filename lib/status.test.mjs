...
test('current-source builds report the same external-module contract used by the runtime', () =>
  withDevelopment(({ manifest, development }) => {
    const build = receipt().receipt;
    const entries = packaged({
      'modex.json': JSON.stringify(
        metadataFor(mods, {
          ...build,
          source: { ...build.source, version: 'new-stock', build: 'new-build' },
          development: { ...development, source: manifest.source },
        }),
      ),
    });
    withApp(entries, (app) => assert.equal(readStatus(app).development.status, 'ready'));
  }));
