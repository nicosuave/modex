...
function receipt() {
  return metadataFor(mods, {
    ...
    transforms: [
      {mod: 'theme-icon', name: 'first', path: 'renderer.js', beforeHash: hash('stock'), afterHash: hash('intermediate')},
      {mod: 'theme-icon', name: 'second', path: 'renderer.js', beforeHash: hash('intermediate'), afterHash: hash('final')},
      {mod: 'theme-icon', name: 'runtime', path: 'modex/runtime.cjs', beforeHash: null, afterHash: hash('runtime')},
    ],
  });
}
function withApp(entries, callback) {
  ...
  try {
    ...
    fs.writeFileSync(seed, Buffer.concat([prefix, json, Buffer.alloc(payloadLength - 4 - json.length)]));
    const filename = path.join(resource, 'app.asar');
    rewriteArchive(seed, filename, new Map(Object.entries(entries).map(([name, bytes]) => [name, Buffer.from(bytes)])));
    callback(app, filename);
  } finally { fs.rmSync(directory, {recursive: true, force: true}); }
}
function packaged(extra = {}) {
  return {'modex.json': JSON.stringify(receipt()), 'renderer.js': 'final', 'modex/runtime.cjs': 'runtime', ...extra};
}

test('status validates final composed output and leaves the installed archive unchanged', () => withApp(packaged(), (app, filename) => {
...
test('status identifies modified files without losing the receipt', () => withApp(packaged({'renderer.js': 'changed'}), app => {
  const status = readStatus(app);
...
test('malformed receipts fail explicitly instead of passing as legacy', () => withApp({'modex.json': '{'}, app => {
...
function withDevelopment(callback) {
  ...
  try {
    ...
    const manifest = {schemaVersion: 1, hookHash: hash('hooks'), mods,
      source: receipt().receipt.source,
      modules: {'theme-icon-render': {file: 'render.cjs', hash: hash(bytes)}, 'theme-icon-runtime': {file: 'runtime.js', hash: hash(bytes)}}};
    const publish = () => fs.writeFileSync(path.join(root, 'manifest.json'), JSON.stringify(manifest));
    publish();
    const development = {root, mods, hookHash: manifest.hookHash, moduleHashes: Object.fromEntries(Object.entries(manifest.modules).map(([id,entry]) => [id, entry.hash]))};
    const entries = packaged({'modex.json': JSON.stringify(metadataFor(mods, {...receipt().receipt, development}))});
    callback({root, manifest, publish, entries, development});
  } finally {fs.rmSync(root, {recursive: true, force: true});}
}

test('status reports configured development bytes without executing or modifying them', () => withDevelopment(({root, entries}) => withApp(entries, app => {
  ...
  assert.ok(result.development.modules.every(module => module.changedSincePackaging === false));
  assert.deepEqual(fs.readFileSync(path.join(root, 'manifest.json')), before);
})));

test('status recognizes rebuilt external modules independently of packaged files', () => withDevelopment(({root, manifest, publish, entries}) => withApp(entries, app => {
  ...
  assert.equal(result.development.status, 'ready');
  assert.deepEqual(result.development.modules.map(module => module.changedSincePackaging), [true, false]);
})));

test('status distinguishes missing development root from incompatible hooks and modified module bytes', () => withDevelopment(({root, manifest, publish, entries}) => withApp(entries, app => {
  manifest.hookHash = hash('different hooks'); publish();
  assert.equal(readStatus(app).development.status, 'incompatible');
  manifest.hookHash = hash('hooks'); publish();
...
test('development provenance remains optional and unknown comparisons are explicit', () => withDevelopment(({development}) => {
  const withoutHashes = {...development}; delete withoutHashes.moduleHashes;
  withApp(packaged({'modex.json': JSON.stringify(metadataFor(mods, {...receipt().receipt, development: withoutHashes}))}), app => {
    assert.ok(readStatus(app).development.modules.every(module => module.changedSincePackaging === null));
  });
