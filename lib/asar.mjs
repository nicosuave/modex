...
export function readArchive(filename) {
  ...
  try {
    ...
    if (prefix.readUInt32LE(0) !== 4 || headerSize < 8 || jsonSize > headerSize - 8 || headerSize > 64 * 1024 * 1024) {
...
function validRelative(filename) {
  return filename && !filename.includes('\\') && !filename.startsWith('/') &&
    filename.split('/').every((part) => part && part !== '.' && part !== '..' && part !== '__proto__' && part !== 'constructor' && part !== 'prototype');
}
...
export function readEntry(archive, filename) {
  ...
  if (!entry || entry.files || entry.link || entry.unpacked) throw new Error(`Not a packed file: ${filename}`);
...
export function rewriteArchive(source, destination, replacements) {
  ...
  for (const [name, bytes] of replacements) {
    ...
    for (const part of parts.slice(0, -1)) {
      if (node.files[part] && (!node.files[part].files || node.files[part].unpacked)) throw new Error(`Overlay conflicts with existing file or unpacked directory: ${name}`);
      node.files[part] ??= { files: {} };
      node = node.files[part];
    }
    const leaf = parts.at(-1);
    const old = node.files[leaf];
    if (old?.files || old?.link || old?.unpacked) throw new Error(`Cannot replace directory, link or unpacked entry: ${name}`);
    node.files[leaf] = { ...old, size: bytes.length, offset: '0', integrity: integrityForBuffer(bytes) };
  }
  ...
  function visit(node, prefix = '') {
    for (const [name, entry] of Object.entries(node.files)) {
      ...
      if (entry.files) visit(entry, `${filename}/`);
        ...
        if (!Number.isSafeInteger(entry.size) || entry.size < 0) throw new Error(`Invalid file size: ${filename}`);
        files.push({ name: filename, size: entry.size, sourceOffset: original ? archive.dataOffset + Number(original.offset) : null });
  ...
  try {
    ...
    for (const file of files) {
      ...
      if (replacement) writeAll(output, replacement);
        ...
        for (let read = 0; read < file.size;) {
          const count = fs.readSync(input, buffer, 0, Math.min(buffer.length, file.size - read), file.sourceOffset + read);
    ...
    return { headerHash: readArchive(destination).headerHash, replaced: [...replacements.keys()], packedFiles: files.length };
  } finally {
