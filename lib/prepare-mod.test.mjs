...
import {
  compatibilityEntryPath,
  compatibilityResourcePath,
  inspectCompatibility,
} from './prepare-mod.mjs';
...
function fixture(fn) {
  ...
  try {
    const source = path.join(directory, 'Test.app'),
      resources = path.join(source, 'Contents/Resources');
    fs.mkdirSync(resources, { recursive: true });
    const bundle = Buffer.from('export const fixture=true;');
    const header = {
      files: {
        webview: {
          files: { assets: { files: { 'app.js': { size: bundle.length, offset: '0' } } } },
        },
        '.vite': {
          files: { build: { files: { 'main.js': { size: bundle.length, offset: '0' } } } },
        },
      },
    };
    ...
    fs.writeFileSync(
      ...
      Buffer.concat([prefix, json, Buffer.alloc(payloadSize - 4 - json.length), bundle]),
    );
    fs.writeFileSync(
      ...
      JSON.stringify({
        ...
        CFBundleVersion: '2',
        ElectronAsarIntegrity: { 'Resources/app.asar': { hash: digest(json) } },
      }),
    );
    ...
    const manifest = {
      version: '1',
      build: '2',
      files: { 'app.js': digest(bundle), '.vite/build/main.js': digest(bundle) },
      resources: { 'icon.png': digest(icon) },
    };
...
test('compatibility validates exact resources and preserves manifest bundle keys', () =>
  fixture(({ source, manifest, bundle }) => {
...
test('changed resource bytes fail compatibility without modifying them', () =>
  fixture(({ source, resources, manifest }) => {
...
test('resource traversal and escaping symlinks fail compatibility', () =>
  fixture(({ source, resources, manifest, directory }) => {
    assert.throws(
      () =>
        inspectCompatibility(source, { ...manifest, resources: { '../Info.plist': 'invalid' } }),
    ...
    assert.throws(
      () =>
        inspectCompatibility(source, {
          ...manifest,
          resources: { 'linked.png': digest('external icon') },
        }),
      /escapes app resources/,
    );
  }));

test('current-source inspection accepts new versions and changed module/resource hashes', () =>
  fixture(({ source, manifest, resources, bundle }) => {
    const changed = {
      ...manifest,
      version: 'older',
      build: 'older',
      files: Object.fromEntries(Object.keys(manifest.files).map((name) => [name, 'old hash'])),
      resources: { 'icon.png': 'old hash' },
    };
    fs.writeFileSync(path.join(resources, 'icon.png'), 'new icon');
    assert.throws(() => inspectCompatibility(source, changed), /Unsupported app/);
    const result = inspectCompatibility(source, changed, { currentSource: true });
    assert.equal(result.info.CFBundleShortVersionString, '1');
    assert.deepEqual(result.bundles.get('app.js'), bundle);
    assert.equal(fs.readFileSync(path.join(resources, 'icon.png'), 'utf8'), 'new icon');
    const plist = path.join(source, 'Contents/Info.plist');
    const info = JSON.parse(fs.readFileSync(plist, 'utf8'));
    info.ElectronAsarIntegrity['Resources/app.asar'].hash = 'corrupt';
    fs.writeFileSync(plist, JSON.stringify(info));
    assert.throws(
      () => inspectCompatibility(source, changed, { currentSource: true }),
      /integrity/,
    );
  }));
