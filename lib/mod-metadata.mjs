...
function hash(value,label) {
  if(typeof value!=='string'||!/^[a-f0-9]{64}$/.test(value))throw Error(`Invalid ${label} SHA256`);
  return value;
}
...
function archivePath(value) {
  ...
  if(value.includes('\\')||value.split('/').some(part=>!part||['.','..','__proto__','constructor','prototype'].includes(part))||value===metadataPath)throw Error('Invalid transform path');
  return value;
}
...
export function metadataFor(mods,receipt) {
  ...
  if(build.revision!==null&&(typeof build.revision!=='string'||!/^[a-f0-9]{40,64}$/.test(build.revision)))throw Error('Invalid build revision');
  if(build.dirty!==null&&typeof build.dirty!=='boolean')throw Error('Invalid build dirty state');
  if(!Array.isArray(receipt.transforms)||!receipt.transforms.length)throw Error('Invalid receipt transforms');
  const names=new Set();
  const latest=new Map();
  const transforms=receipt.transforms.map(value=>{
    const record=object(value,'transform');
    if(!selected.includes(record.mod)&&!['app-tools-auth','development'].includes(record.mod))throw Error('Unknown transform owner');
    ...
    names.add(key);
    const beforeHash=record.beforeHash===null?null:hash(record.beforeHash,'transform before');
    const afterHash=hash(record.afterHash,'transform after');
    if(latest.has(filename)&&latest.get(filename)!==beforeHash)throw Error(`Broken transform hash chain: ${filename}`);
    latest.set(filename,afterHash);
    return {mod:record.mod,name,path:filename,beforeHash,afterHash};
  });
  const result={schemaVersion:2,mods:selected,receipt:{
    source:{version:text(source.version,'source version'),build:text(source.build,'source build'),asarHash:hash(source.asarHash,'source archive')},
    build:{revision:build.revision,dirty:build.dirty,sourceHash:hash(build.sourceHash,'build source')},transforms,
  }};
  if(receipt.development!==undefined) {
    ...
    if(!root.startsWith('/')||root.includes('\0'))throw Error('Development root must be absolute');
    const developmentMods=validateMods(development.mods);
    if(developmentMods.some(id=>!selected.includes(id)))throw Error('Development mod is not selected');
    result.receipt.development={root,mods:developmentMods};
    if(development.hookHash!==undefined)result.receipt.development.hookHash=hash(development.hookHash,'development hook');
    if(development.moduleHashes!==undefined) {
      ...
      result.receipt.development.moduleHashes=Object.fromEntries(Object.entries(hashes).map(([id,value])=>{
