...
function withArchive(entries, fn) {
  ...
  try {
    ...
    fs.writeFileSync(
      filename,
      Buffer.concat([prefix, json, Buffer.alloc(payloadLength - 4 - json.length), ...payloads]),
    );
...
for (const mods of [['model-spread'], ['theme-icon'], both]) {
  test(`metadata survives archive packaging for ${mods.join(',')}`, () =>
    withArchive({ 'stock.js': 'unchanged' }, (archive, directory) => {
      ...
      rewriteArchive(
        archive.filename,
        output,
        new Map([['modex.json', Buffer.from(JSON.stringify(metadata))]]),
      );
      assert.deepEqual(readInstalledMods(readArchive(output), markers), mods);
    }));
  test(`legacy marker detection identifies ${mods.join(',')}`, () =>
    withArchive(Object.fromEntries(mods.map((id) => [markers[id], '{}'])), (archive) =>
      assert.deepEqual(readInstalledMods(archive, markers), mods),
    ));
}

test('metadata takes precedence over legacy markers', () =>
  withArchive(
    {
...
test('unknown future mods remain visible for loss detection', () =>
  withArchive(
    {
...
test('rejects an unidentifiable installed archive', () =>
  withArchive({ 'stock.js': '' }, (archive) => {
...
test('rejects malformed metadata rather than falling back to legacy markers', () => {
  ...
  for (const metadata of invalid)
    withArchive({ 'modex.json': metadata, [markers['model-spread']]: '{}' }, (archive) => {
...
const receipt = {
  ...
  transforms: [
    ...
    },
    {
      mod: 'theme-icon',
      name: 'runtime',
      path: 'modex/theme.cjs',
      beforeHash: null,
      afterHash: '6'.repeat(64),
    },
  ],
};

test('schema2 round-trips build facts and preserves mod selection', () =>
  withArchive(
    {
    ...
    (archive) => {
      ...
      assert.equal(
        JSON.stringify(metadataFor(both, receipt)),
        JSON.stringify(metadataFor(both, receipt)),
      );
    },
  ));
...
test('receipt rejects invalid or disconnected evidence instead of claiming provenance', () => {
  const edits = [
    (value) => {
      value.source.asarHash = 'not a hash';
    },
    (value) => {
      value.build.sourceHash = null;
    },
    (value) => {
      delete value.build.dirty;
    },
    (value) => {
      value.build.revision = 'HEAD';
    },
    (value) => {
      value.transforms = [];
    },
    (value) => {
      value.transforms[0].path = '../outside';
    },
    (value) => {
      value.transforms[0].path = 'modex.json';
    },
    (value) => {
      value.transforms[0].mod = 'unknown-mod';
    },
    (value) => {
      value.transforms[0].afterHash = null;
    },
    (value) => {
      value.transforms[1].beforeHash = '0'.repeat(64);
    },
    (value) => {
      value.transforms.push(value.transforms[2]);
    },
    (value) => {
      value.development = { root: 'relative', mods: ['theme-icon'] };
    },
    (value) => {
      value.development = { root: '/tmp/dev', mods: ['unknown-mod'] };
    },
  ];
  for (const edit of edits) {
    ...
    withArchive(
      { 'modex.json': JSON.stringify({ schemaVersion: 2, mods: both, receipt: value }) },
      (archive) => {
...
test('development provenance preserves optional hook and known module hashes', () => {
  ...
  value.development = {
    root: '/tmp/dev',
    mods: ['theme-icon'],
    hookHash: '7'.repeat(64),
    moduleHashes: { 'theme-icon-render': '8'.repeat(64) },
  };
