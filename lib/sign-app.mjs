...
export function signingIdentity(
  requested = process.env.CODEX_MODS_SIGN_IDENTITY ?? process.env.MODEL_SPREAD_SIGN_IDENTITY,
) {
  const available = command('/usr/bin/security', ['find-identity', '-v', '-p', 'codesigning']);
  const identities = [
    ...available.matchAll(/\b([A-F0-9]{40}) "(Developer ID Application: [^"]+)"/g),
  ].map(([, hash, name]) => ({ hash, name }));
  const matches = requested
    ? identities.filter((x) => x.name === requested || x.hash === requested)
    : identities;
  if (matches.length !== 1)
    throw Error(
      'Select exactly one installed Developer ID Application certificate with CODEX_MODS_SIGN_IDENTITY; ad-hoc signing is not allowed',
    );
  return matches[0];
}
export function designatedRequirement(app) {
  ...
  if (
    ...
    throw Error('App lacks a stable certificate-based designated requirement');
  return requirement;
}
export function validateAppIdentity(appName, bundleID) {
  if (
    ...
    /[\x00-\x1f/]/.test(appName)
  )
    throw Error('A nonempty app name without path separators is required');
  if (typeof bundleID !== 'string' || !/^local\.codex\.[a-z0-9]+(?:[.-][a-z0-9]+)*$/.test(bundleID))
    throw Error('Use a unique stable bundle ID under local.codex, separate from the stock app');
}
export function signApp(
  app,
  { appName, bundleID, identity, home = os.homedir(), original = '/Applications/ChatGPT.app' } = {},
) {
  ...
  if (info.CFBundleIdentifier !== 'com.openai.codex' && info.CFBundleIdentifier !== bundleID)
    throw Error('Expected a staged stock Codex app or the same mod identity');
  ...
  try {
    ...
    fs.writeFileSync(
      entitlements,
      xml.slice(xml.indexOf('<?xml') >= 0 ? xml.indexOf('<?xml') : xml.indexOf('<plist')),
    );
    const values = JSON.parse(
      command('/usr/bin/plutil', ['-convert', 'json', '-o', '-', entitlements]),
    );
    ...
    command('/usr/bin/plutil', [
      ...
      JSON.stringify({
        ...
        CODEX_ELECTRON_USER_DATA_PATH: path.join(home, 'Library/Application Support/Codex'),
      }),
      plist,
    ]);
    ...
    if (!requirement.includes(`identifier "${bundleID}"`))
      throw Error('Unexpected signed app identity');
    return {
      app: target,
      identity: signer.name,
      bundleID,
      requirement,
      removedEntitlements,
      addedEntitlements: ['com.apple.security.cs.disable-library-validation'],
    };
  } finally {
...
if (import.meta.main) {
  try {
    const [app, appName, bundleID] = process.argv.slice(2);
    if (!app)
      throw Error(
        'Usage: bun lib/sign-app.mjs /path/to/staged/Mod.app "Mod Name" local.codex.mod-name',
      );
    console.log(JSON.stringify(signApp(app, { appName, bundleID }), null, 2));
  } catch (error) {
