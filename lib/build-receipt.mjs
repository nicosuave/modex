...
export function recordChanges(mod, name, before, after) {
  ...
  return [...output]
    .filter(
      ([filename, bytes]) =>
        filename !== 'modex.json' &&
        (!input.has(filename) || sha256(input.get(filename)) !== sha256(bytes)),
    )
    ...
    .map(([filename, bytes]) => ({
      ...
      name,
      path: filename,
      beforeHash: input.has(filename) ? sha256(input.get(filename)) : null,
...
export function buildIdentity(root = path.resolve(import.meta.dirname, '..')) {
  ...
  try {
    ...
    for (const name of ['modex.mjs', 'package.json', 'bun.lock'])
  ...
  const selected = [...new Set(files)]
    .filter(
      (name) =>
        /^(?:lib\/|mods\/|modex\.mjs$|package\.json$|bun\.lock$)/.test(name) &&
        /\.(?:mjs|cjs|js|json|cpp|h|lock)$/.test(name) &&
        !name
          .split('/')
          .some((part) => ['node_modules', 'bundles', 'overlay', 'dist', 'work'].includes(part)),
    )
    .sort();
  for (const name of selected) {
    const filename = path.join(root, name);
    if (!fs.existsSync(filename)) {
      digest.update(name + '\0deleted\0');
      continue;
    }
    if (!fs.lstatSync(filename).isFile())
      throw Error(`Build source must be a regular file: ${name}`);
    digest
      .update(name + '\0')
